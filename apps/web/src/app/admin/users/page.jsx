...
const ROLES = [
  {
    value:"admin",
    label:"Admin",
    desc:"Full access — clients, equipment, certificates, users",
    color:T.accent
  },
  {
    value:"inspector",
    label:"Inspector",
    desc:"Can create and edit certificates, NCRs, CAPAs",
    color:T.green
  },
  {
    value:"viewer",
    label:"Viewer",
    desc:"Read-only — can view and print certificates",
    color:T.textMid
  },
];
...
export default function AdminUsersPage() {
  ...
  async function handleDelete(userId, name) {
    if (
      !window.confirm(
        `Permanently delete ${name}? This cannot be undone and will remove them from the system completely.`
      )
    ) {
      return;
    }
      ...
      >
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                borderRadius:11,
                border:"none",
                background:showForm ? "rgba(255,255,255,0.06)" : "linear-gradient(135deg,#22d3ee,#0891b2)",
                color:showForm ? T.textMid : "#052e16",
                fontWeight:900,
                fontSize:13,
                cursor:"pointer",
                fontFamily:"'IBM Plex Sans',sans-serif"
        ...
        {showForm && (
          ...
          >
            ...
            <form onSubmit={handleCreate}>
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => hf("full_name", e.target.value)}
                ...
                <div>
                  ...
                  <select
                    style={IS}
                    value={form.role}
                    onChange={(e) => hf("role", e.target.value)}
                  >
                    {ROLES.map((r) => (
              ...
              >
                <strong style={{ color:T.accent }}>📧 Invite flow:</strong> The user will receive an{" "}
                <strong style={{ color:T.text }}>email invitation</strong> with a link to set their own password.
                They cannot log in until they click the link and create their password.
                Their email is added to the approved list automatically.
              </div>
        ...
        >
          {loading ? (
            ...
            <div style={{ padding:40, textAlign:"center", color:T.textDim, fontSize:13 }}>
              No users yet. Click "Invite User" to add the first user.
            </div>
          ) : (
            <>
                ...
                <table className="usr-table">
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr
                        key={u.id}
                        className={u.status === "inactive" ? "usr-row-inactive" : ""}
                      >
                        <td>
                        ...
                        <td>
                          <select
                            ...
                            style={{
                              ...
                              border:"none",
                              color:u.role === "admin" ? T.accent : u.role === "inspector" ? T.green : T.textMid,
                              fontWeight:800,
                              fontSize:12,
                              cursor:"pointer",
                              fontFamily:"'IBM Plex Sans',sans-serif",
                          ...
                          >
                            {ROLES.map((r) => (
                              <option
                                key={r.value}
                                value={r.value}
                                style={{ background:"#0a1420", color:"#f0f6ff" }}
                              >
                                {r.label}
                              </option>
                            ))}
                          ...
                          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                            ...
                            <button
                              ...
                              style={{
                                padding:"5px 10px",
                                borderRadius:7,
                                border:`1px solid ${u.status === "active" ? T.redBrd : T.greenBrd}`,
                                background:u.status === "active" ? T.redDim : T.greenDim,
                                color:u.status === "active" ? T.red : T.green,
                                fontSize:11,
                                fontWeight:700,
                                cursor:"pointer",
                                fontFamily:"inherit",
