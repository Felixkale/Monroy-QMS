...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ minHeight:"100vh", backgroundColor:"#0f1419", color:"#e2e8f0", display:"flex", flexDirection:"column" }}>
      <div style={{
            ...
            <div>
              ...
              <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(124,92,252,0.2)", borderRadius:16, padding:20 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12 }}>
                  {["Add Client","Edit Client","Delete Client","Add Equipment","Manage Users","View Logs","Assign Roles","Generate Report"].map((action,idx)=>(
            ...
            <div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                ...
                <button style={{
                  ...
                  border:"none", color:"#fff", fontWeight:700, cursor:"pointer", fontFamily:"inherit", fontSize:12,
                }}>➕ Add Client</button>
              </div>
              <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(124,92,252,0.2)", borderRadius:16, padding:20, overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                  <thead>
                    <tr style={{ borderBottom:"2px solid rgba(124,92,252,0.2)" }}>
                      {["Client Name","Industry","Equipment","Status","Actions"].map(h=>(
                        <th key={h} style={{ padding:12, textAlign:"left", color:"#7c5cfc", fontWeight:700 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={idx} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                        ...
                        <td style={{ padding:12, color:C.green }}>{c.equipment}</td>
                        <td style={{ padding:12 }}>
                          <span style={{ padding:"2px 8px", borderRadius:12, fontSize:10, fontWeight:700, background:"rgba(0,245,196,0.1)", color:C.green }}>{c.status}</span>
                        </td>
                        <td style={{ padding:12, display:"flex", gap:6 }}>
                          <button style={{ padding:"4px 10px", borderRadius:6, background:"rgba(124,92,252,0.15)", border:"none", color:C.purple, fontWeight:600, fontSize:10, cursor:"pointer", fontFamily:"inherit" }}>Edit</button>
                          <button style={{ padding:"4px 10px", borderRadius:6, background:"rgba(244,114,182,0.15)", border:"none", color:C.pink, fontWeight:600, fontSize:10, cursor:"pointer", fontFamily:"inherit" }}>Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
