...
//                   Air Receiver, Safety Harness / Lanyard
//                   Batch row table — 10–14 units/page
//
...
//                   Shared header + 4 units/page
//
//   SINGLE        → Pressure Vessel, Boiler, Autoclave
//                   Full A4 — 1 unit/page
// ─────────────────────────────────────────────────────────────────────────────
...
// Exact-match table types — batch row format
// NOTE: "air receiver" and "safety harness"/"harness"/"lanyard" are now TABLE too
const TABLE_TYPES = [
  "wire rope sling", "chain sling", "bottle jack",
  "air receiver",
  "safety harness", "harness", "lanyard", "rope absorber", "shock absorber",
];

// Single full-A4 page — pressure systems only (air receiver removed — now TABLE)
const SINGLE_TYPES = [
  "pressure vessel", "boiler", "autoclave",
...
const TACKLE_KEYWORDS = [
  ...
  "fork arm",
  "wire rope",
  "clamp", "spreader beam", "lift beam", "lifting beam", "crawl beam",
  "lifting accessory", "universal lifter",
  "webbing sling", "round sling", "wire sling", "polyester sling",
  "flat web sling", "multi-leg chain sling", "4-leg chain sling",
  "endless round sling",
...
function routeType(raw) {
  ...
  // 1. Multi-leg / 4-leg / endless slings → tackle
  if (
    t.includes("multi-leg") || t.includes("4-leg") || t.includes("4 leg") ||
    t.includes("multi leg")  || t.includes("endless")
  ) return "quad-tackle";

  // 2. TABLE — batch row: wire rope sling, chain sling, bottle jack,
  //            air receiver, safety harness / harness / lanyard
  ...
  // 4. QUAD-TACKLE — lifting tackle: no Reg, no Fleet, no PV
  if (TACKLE_KEYWORDS.some(k => t.includes(k))) return "quad-tackle";

  // 5. QUAD-MACHINE — everything else
  return "quad-machine";
}
...
function SB({ title, children, flex, mb = 3, s }) {
  return (
    <div style={{ border: "1px solid #ccc", marginBottom: mb, flex: flex || undefined, minHeight: 0, ...s }}>
      <div style={{
          ...
          <div key={i} style={{ flex: 1, border: "1px solid #ccc", padding: "2px 4px" }}>
            {cols.map((lbl, j) => (
              <div key={j} style={{ marginBottom: j < 2 ? 3 : 0 }}>
                ...
                <span style={{
                  display: "inline-block", borderBottom: `1px solid ${LINE}`,
                  width: "55%", marginLeft: 2,
                }}>&nbsp;</span>
              </div>
            ))}
          </div>
...
function MachineMiniUnit({ n, equipType }) {
  ...
  return (
    <div style={{
      border: `1.5px solid ${NAVY}`, borderRadius: 2, display: "flex",
...
/* ══════════════════════════════════════════════════════════════
   TABLE COLUMN + CHECKLIST CONFIG
   — covers Wire Rope Sling, Chain Sling, Bottle Jack,
     Air Receiver, Safety Harness
══════════════════════════════════════════════════════════════ */
const TH_S = {
...
function getTableConfig(t) {
  /* ── AIR RECEIVER ─────────────────────────────────────────── */
  if (t.includes("air receiver")) {
    return {
      rows: 10,
      cols: [
        { l: "Asset / Tag No.",         w: 52, mono: true },
        { l: "Serial No.",              w: 52, mono: true },
        { l: "Vol. (L)",                w: 28 },
        { l: "MAWP (bar)",              w: 30 },
        { l: "Test Press. (bar)",       w: 34 },
        { l: "SRV Set (bar)",           w: 30 },
        { l: "Next Test Due",           w: 38 },
        { l: "SRV Popped",              w: 30 },
        { l: "Auto Drain OK",           w: 32 },
      ],
      checks: [
        ...
        "Moisture trap / separator condition",
        "Pressure switch — cut-in & cut-out set",
        "Compressor unload cycle correct",
      ...
      legal: "Inspection performed per Mines, Quarries, Works & Machinery Act Cap 44:02 — Pressure Vessels Regulations.",
      note:  "Record test pressure held ≥ 30 min. SRV pop test mandatory. Attach hydrostatic test certificate where applicable.",
    };
  }

  /* ── SAFETY HARNESS / LANYARD ─────────────────────────────── */
  if (
    t.includes("safety harness") || t.includes("harness") ||
    t.includes("lanyard") || t.includes("rope absorber") || t.includes("shock absorber")
  ) {
    return {
      rows: 14,
      cols: [
        { l: "Tag / Serial No.",        w: 54, mono: true },
        { l: "Type / Model",            w: 52 },
        { l: "Manufacturer",            w: 48 },
        { l: "Mfg. Date",               w: 32 },
        { l: "First Use",               w: 32 },
        { l: "Max SWL (kg)",            w: 30 },
        { l: "Deployed (Y/N)",          w: 30 },
        { l: "Next Insp. Due",          w: 34 },
      ],
      checks: [
        "Webbing — no cuts, abrasion, heat damage or UV degradation",
        "Stitching — no broken or pulled threads at all load-bearing seams",
        "Buckles — correct type, function, no cracks or deformation",
        "D-rings & anchorage points — no cracks, corrosion or deformation",
        "Snap hooks — gate closes fully, no burring or deformation",
        "Locking mechanism — auto-locks, no play or sticking",
        "Energy absorber pack — sealed, undamaged, NOT previously deployed",
        "Deployment indicator / witness thread intact",
        "Lanyard rope or webbing — no kinks, fraying or chemical damage",
        "Connecting carabiner / karabiner — OK & cross-gate loaded",
        "Labels — CE/SABS mark, traceability, date of manufacture legible",
        "Storage & environmental exposure — no chemical or UV damage noted",
        "Post-shock-load retirement check — decommissioned if deployed",
        "User weight within rated SWL",
      ],
      legal: "Inspection per Factories Act & BS EN 361, EN 354, EN 355 / ANSI/ASSE Z359 standards.",
      note:  "ANY harness or lanyard that has arrested a fall MUST be immediately withdrawn from service and destroyed. Record destruction date.",
    };
  }
...
/* ══════════════════════════════════════════════════════════════
   TABLE TEMPLATE
   Wire Rope Sling, Chain Sling, Bottle Jack — 14 rows
   Air Receiver                              — 10 rows (wider cols)
   Safety Harness / Lanyard                  — 14 rows
══════════════════════════════════════════════════════════════ */
function TableTemplate({ equipType }) {
  const t      = (equipType || "").toLowerCase();
  const cfg    = getTableConfig(t);
  const { rows, cols, checks, legal, note } = cfg;

  const isAR      = t.includes("air receiver");
  const isHarness = t.includes("harness") || t.includes("lanyard") ||
                    t.includes("rope absorber") || t.includes("shock absorber") ||
                    t.includes("safety harness");
  ...
  return (
    <div className="a4-page" style={pageStyle}>
      ...
      {/* ── Air Receiver: add a second metadata row for compressor/area ── */}
      {isAR && (
        <div style={{ display: "flex", gap: 8, marginBottom: 5 }}>
          <WL label="Compressor Make / Model" flex={2} />
          <WL label="Compressor Serial No."   flex={1.5} mono />
          <WL label="Area / Department"        flex={1.5} />
          <WL label="Inspection Type (Routine / Periodic / Major)" flex={2} />
        </div>
      )}

      {/* ── Safety Harness: add user assignment / work area row ── */}
      {isHarness && (
        <div style={{ display: "flex", gap: 8, marginBottom: 5 }}>
          <WL label="Department / Work Area"         flex={2} />
          <WL label="Inspection Frequency"           flex={1.5} />
          <WL label="Standard Applied (EN / ANSI)"   flex={2} />
          <WL label="Previous Inspection Date"        flex={1.5} />
        </div>
      )}
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 7.5, marginBottom: 5 }}>
        <thead>
          <tr style={{ background: NAVY, color: WHITE }}>
            ...
            {cols.map((c, i) => (
              <th key={i} style={{ ...TH_S, width: c.w, fontFamily: c.mono ? "monospace" : "inherit" }}>
                {c.l}
              </th>
            ))}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 5 }}>
        <SB
          title="INSPECTION CHECKLIST — applies to all units above   ☐ Pass  ☐ Fail"
          flex={isAR ? 1.4 : 1.2}
          mb={0}
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ flex: 1 }}>
              {checks.slice(0, Math.ceil(checks.length / 2)).map((item, i) => <CK key={i} label={item} />)}
            </div>
            <div style={{ flex: 1 }}>
              {checks.slice(Math.ceil(checks.length / 2)).map((item, i) => <CK key={i} label={item} />)}
            </div>
          </div>
        </SB>

        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 5 }}>
          <SB title="REMARKS / DEFECTS  (reference row # from table)" flex={1} mb={0} s={{ flex: 1 }}>
            {Array.from({ length: isAR ? 6 : 9 }).map((_, i) => (
              <div key={i} style={{ borderBottom: "1px solid #ddd", minHeight: 12, marginBottom: 3 }} />
            ))}
          </SB>

          {/* Air Receiver: test data summary box */}
          {isAR && (
            <div style={{ border: "1px solid #ccc", padding: "3px 5px" }}>
              <div style={{
                background: NAVY, color: WHITE, fontWeight: 700,
                fontSize: 6.5, padding: "1.5px 4px", letterSpacing: 0.3, marginBottom: 4,
              }}>PRESSURE TEST SUMMARY</div>
              <WR fields={[{ l: "Test Method", f: 1.5 }, { l: "Test Equip. Ref", f: 1 }]} />
              <WR fields={[{ l: "Test Duration", f: 1 }, { l: "Overall Result", f: 1 }]} mb={0} />
            </div>
          )}

          {/* Harness: decommission log */}
          {isHarness && (
            <div style={{ border: "1px solid #ccc", padding: "3px 5px" }}>
              <div style={{
                background: RED, color: WHITE, fontWeight: 700,
                fontSize: 6.5, padding: "1.5px 4px", letterSpacing: 0.3, marginBottom: 4,
              }}>⚠ DECOMMISSIONED / DESTROYED (post-fall)</div>
              <WR fields={[{ l: "Row # / Tag No.", f: 1 }, { l: "Reason", f: 1.5 }, { l: "Date", f: 1 }]} />
              <WR fields={[{ l: "Row # / Tag No.", f: 1 }, { l: "Reason", f: 1.5 }, { l: "Date", f: 1 }]} mb={0} />
            </div>
          )}
        </div>
      </div>
      ...
      {legal && (
        <div style={{
          ...
          borderLeft: `4px solid ${NAVY}`, padding: "3px 7px",
          fontSize: 7, marginBottom: 5, color: "#333",
        }}>
          <strong style={{ color: NAVY }}>COMPLIANCE: </strong>{legal}
          {note && <><br /><strong style={{ color: RED }}>NOTE: </strong>{note}</>}
        </div>
      )}
...
/* ══════════════════════════════════════════════════════════════
   SINGLE TEMPLATE — Pressure Vessel / Boiler / Autoclave
...
function PrintPageInner() {
  ...
  const label =
    layout === "table"        ? "Batch table — multiple units per page" :
