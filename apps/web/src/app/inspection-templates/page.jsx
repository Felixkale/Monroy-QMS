...
const T = {
  ...
  purple:"#a78bfa",purpleDim:"rgba(167,139,250,0.08)",purpleBrd:"rgba(167,139,250,0.22)",
};
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box}
  ...
  .it-hero::before{
    ...
    background:radial-gradient(ellipse at right,rgba(34,211,238,0.06),transparent 70%);
    pointer-events:none;
  }
  .it-hero-tag{
  ...
  /* FILTERS */
  .it-filters{
    background:rgba(13,22,38,0.8);border:1px solid rgba(148,163,184,0.12);
  ...
  .it-select,.it-input{
    ...
    border-radius:10px;padding:10px 14px;color:#f0f6ff;font-size:13px;
  ...
  .it-btn{
    ...
    padding:10px 22px;border-radius:11px;font-family:'IBM Plex Sans',sans-serif;
    font-size:13px;font-weight:800;cursor:pointer;border:none;min-height:44px;
    -webkit-tap-highlight-color:transparent;transition:filter .15s,transform .15s;
    white-space:nowrap;
  }
  ...
  .it-results-header{
    padding:16px 20px;border-bottom:1px solid rgba(148,163,184,0.10);
  ...
  .it-count-badge{
    ...
    color:#22d3ee;padding:3px 10px;border-radius:99px;
  }

  .it-table-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch}
  .it-table{width:100%;border-collapse:collapse;font-size:13px}
  .it-table th{
    padding:10px 16px;text-align:left;font-size:9px;font-weight:800;
    letter-spacing:0.12em;text-transform:uppercase;color:rgba(240,246,255,0.35);
    border-bottom:1px solid rgba(148,163,184,0.10);white-space:nowrap;
  }
  .it-table td{
    padding:13px 16px;border-bottom:1px solid rgba(148,163,184,0.07);
    color:rgba(240,246,255,0.85);vertical-align:middle;
  }
  ...
  /* PREVIEW PANEL */
  .it-preview{
    background:rgba(13,22,38,0.8);border:1px solid rgba(34,211,238,0.2);
    border-radius:16px;padding:20px;backdrop-filter:blur(20px);
  ...
  .it-checklist-item{
    ...
    color:rgba(240,246,255,0.65);display:flex;align-items:center;gap:5px;
  }
  .it-checklist-item::before{content:'□';font-size:12px;color:rgba(240,246,255,0.3)}

  /* EMPTY / LOADING STATES */
  .it-empty{
    padding:60px 20px;text-align:center;color:rgba(240,246,255,0.28);
  ...
  .it-spinner{
    ...
    border:2px solid rgba(34,211,238,0.15);border-top-color:#22d3ee;
  ...
  @media(max-width:768px){
    ...
    .it-filters{grid-template-columns:1fr 1fr;gap:10px}
    .it-filters .it-btn{grid-column:1/-1}
    .it-table th:nth-child(4),.it-table td:nth-child(4),
...
// Equipment type checklist definitions
const CHECKLISTS = {
...
function getChecklist(equipType) {
  ...
  if (t.includes("SLING") || t.includes("WIRE SLING")) return CHECKLISTS.WIRE_SLING;
  if (t.includes("PRESSURE") || t.includes("VESSEL") || t.includes("COMPRESSOR")) return CHECKLISTS.PRESSURE_VESSEL;
  if (t.includes("FORKLIFT") || t.includes("FORK LIFT")) return CHECKLISTS.FORKLIFT;
...
export default function InspectionTemplatesPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from("certificates")
        .select("client_name, equipment_type")
        .order("client_name");
      if (!data) return;
      const cls = [...new Set(data.map(r => r.client_name).filter(Boolean))].sort();
      const eqs = [...new Set(data.map(r => r.equipment_type).filter(Boolean))].sort();
      setClients(cls);
      setEquipTypes(eqs);
    })();
  }, []);
  ...
  async function handleSearch() {
    ...
    let q = supabase
      .from("certificates")
      .select("id, certificate_number, client_name, equipment_type, equipment_description, serial_number, manufacturer, model, swl, working_pressure, location, inspection_date, expiry_date, result, fleet_number, reg_number")
      .order("inspection_date", { ascending: false })
      .limit(200);

    if (selClient !== "ALL") q = q.eq("client_name", selClient);
  ...
  function openPrint() {
    if (!selected) return;
    window.open(`/inspection-templates/print?certId=${selected.id}`, "_blank");
  }

  const checklist = selected ? getChecklist(selected.equipment_type) : [];
  const typeColor = selected ? getTypeColor(selected.equipment_type) : {};
      ...
      <div className="it-page">
        <div className="it-wrap">
          ...
          <div className="it-hero">
            ...
            <p>Generate printable blank inspection forms pre-filled with equipment data from existing certificates. Select a certificate below to create its inspection template.</p>
          </div>

          {/* FILTERS */}
          <div className="it-filters">
            <div className="it-field">
              <span className="it-label">Client</span>
              <select className="it-select" value={selClient} onChange={e => setSelClient(e.target.value)}>
          ...
          {searched && (
            <div style={{ display:"flex", gap:12, alignItems:"center" }}>
              <div className="it-field" style={{ flex:1 }}>
                <span className="it-label">Filter by description</span>
                <input
                  className="it-input"
              ...
              <button type="button" className="it-btn it-btn-ghost" style={{ marginTop:21 }} onClick={handleSearch}>Apply</button>
            </div>
          )}
            ...
            <div className="it-results">
              ...
              {certs.length === 0 ? (
                <div className="it-empty">
                  ...
                  No certificates match your filters.<br/>Try selecting a different client or equipment type.
                </div>
              ) : (
                      ...
                      {certs.map(c => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:12, color:"rgba(240,246,255,0.55)" }}>
          ...
          {!searched && !loading && (
            <div className="it-empty">
              ...
              Select filters above and click <strong>Search</strong> to find certificates.<br/>
              Then pick one to preview its inspection template.
            </div>
          )}
            ...
            <div className="it-preview">
              <div className="it-preview-header">
                <div>
                  <div className="it-preview-title">
                    {selected.equipment_description || selected.equipment_type || "Equipment"}
                  </div>
                  <div className="it-preview-sub">
                    {selected.certificate_number} · {selected.client_name}
                  </div>
                </div>
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  <button type="button" className="it-btn it-btn-ghost" onClick={() => setSelected(null)}>
                    ✕ Clear
                  </button>
                  <button type="button" className="it-btn it-btn-print" onClick={openPrint}>
                    🖨 Print Template
                  </button>
                </div>
              </div>
              ...
              <div className="it-preview-grid">
                {[
                  ...
                  ["Previous Result",  selected.result],
              ...
              <div className="it-checklist-preview">
                <div className="it-checklist-title">Inspection checklist — {checklist.length} items</div>
                <div className="it-checklist-items">
                  {checklist.map(item => (
                    <div className="it-checklist-item" key={item}>{item}</div>
                  ))}
                </div>
              </div>

              <div style={{ fontSize:11, color:"rgba(240,246,255,0.3)", borderTop:"1px solid rgba(148,163,184,0.08)", paddingTop:12, marginTop:4 }}>
                The printed template will include all pre-filled fields above, blank inspection fields, the checklist with PASS/FAIL/N/A columns, and signature blocks.
              </div>
            </div>
          )}
