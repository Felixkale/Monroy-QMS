...
// ✅ FIX: default inspector + legal framework pre-filled
const initialForm = {
  asset_id:              "",
...
export default function CreateCertificatePage() {
  ...
  const selectedAsset = useMemo(
    () => assets.find((item) => item.id === form.asset_id) || null,
    ...
    async function loadAssets() {
      try {
        ...
        const { data, error } = await supabase
          .from("assets")
          .select(`
            id, asset_name, asset_tag, asset_type, manufacturer,
            model, serial_number, year_built, location,
            safe_working_load, working_pressure,
    ...
    setForm((prev) => {
      ...
      if (name === "asset_id") {
        ...
        if (asset) {
          ...
          // ✅ Use serial_number first, fallback to asset_tag
          next.equipment_id          = asset.serial_number || asset.asset_tag || "";
          ...
          next.legal_framework       = asset.design_standard || "Mines, Quarries, Works and Machinery Act Cap 44:02";
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!form.inspector_name.trim()) throw new Error("Inspector name is required.");
      ...
      setForm(initialForm);
      setTimeout(() => router.push("/certificates"), 800);
    } catch (err) {
        ...
        <div style={styles.headerCard}>
          <div>
            ...
            <p style={styles.subtitle}>
              Fill in the details below. Inspector name and legal framework are pre-filled with defaults.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.section}>
            ...
            <div style={styles.grid3}>
              <SelectField
                label="Equipment *" name="asset_id" value={form.asset_id}
                onChange={handleChange} required
                options={[
                  { value: "", label: assetsLoading ? "Loading equipment..." : "Select equipment" },
                  ...assets.map((a) => ({
                    value: a.id,
                    label: `${a.asset_tag || "NO-TAG"} - ${a.asset_type || a.asset_name || "Equipment"}`,
                  })),
                ]}
              />
              <Field label="Certificate Type *" name="certificate_type" value={form.certificate_type} onChange={handleChange} required />
              <SelectField
                label="Equipment Status" name="equipment_status" value={form.equipment_status}
                onChange={handleChange} options={["PASS", "FAIL", "CONDITIONAL"]}
              />
              <Field label="Issue Date *" name="issued_at" value={form.issued_at} onChange={handleChange} type="date" required />
              <Field label="Expiry Date"  name="valid_to"  value={form.valid_to}  onChange={handleChange} type="date" />
              <SelectField
                label="Record Status" name="status" value={form.status}
                onChange={handleChange} options={["issued", "draft", "expired", "rejected"]}
              />
            </div>
          </div>
          ...
          <div style={styles.section}>
            ...
            <div style={styles.grid3}>
              ...
              <Field label="SWL"   name="swl"  value={form.swl}  onChange={handleChange} placeholder="e.g. 5 Tons" />
              <Field label="MAWP"  name="mawp" value={form.mawp} onChange={handleChange} placeholder="e.g. 1600 kPa" />
              <Field label="Legal Framework" name="legal_framework" value={form.legal_framework} onChange={handleChange} />
              {/* ✅ Pre-filled with Moemedi Masupe */}
              <Field label="Inspector Name *" name="inspector_name" value={form.inspector_name} onChange={handleChange} required />
              {/* ✅ Pre-filled with 700117910 */}
          ...
          <div style={styles.actions}>
            <button type="button" onClick={() => router.push("/certificates")} style={styles.secondaryBtn}>
              Cancel
            </button>
...
const styles = {
  page:        { padding: 24, color: C.text, background: "transparent" },
  headerCard:  { display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap", padding: 20, borderRadius: 20, background: "linear-gradient(135deg, rgba(124,92,252,0.16), rgba(79,195,247,0.10))", border: `1px solid ${C.border}`, marginBottom: 20 },
  kicker:      { color: C.green, fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 8 },
  title:       { margin: 0, fontSize: 30, lineHeight: 1.1 },
  subtitle:    { margin: "8px 0 0", color: C.subtext, maxWidth: 760 },
  previewBox:  { minWidth: 240, padding: 16, borderRadius: 16, background: "rgba(11,16,32,0.7)", border: `1px solid ${C.border}` },
  success:     { marginBottom: 16, padding: "12px 14px", borderRadius: 12, background: "rgba(0,245,196,0.12)", border: "1px solid rgba(0,245,196,0.3)", color: C.green, fontWeight: 700 },
  error:       { marginBottom: 16, padding: "12px 14px", borderRadius: 12, background: "rgba(255,107,107,0.12)", border: "1px solid rgba(255,107,107,0.28)", color: C.danger, fontWeight: 700 },
  form:        { display: "grid", gap: 20 },
  section:     { padding: 20, borderRadius: 18, background: C.panel, border: `1px solid ${C.border}` },
  sectionTitle:{ fontSize: 18, fontWeight: 800, marginBottom: 16 },
  grid3:       { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 },
  input:       { width: "100%", height: 46, borderRadius: 12, border: `1px solid ${C.border}`, background: "#0f1730", color: C.text, padding: "0 14px", outline: "none" },
  actions:     { display: "flex", justifyContent: "flex-end", gap: 12, marginTop: 4 },
  secondaryBtn:{ height: 46, padding: "0 18px", borderRadius: 12, border: `1px solid ${C.border}`, background: "transparent", color: C.text, fontWeight: 700, cursor: "pointer" },
  primaryBtn:  { height: 46, padding: "0 18px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #7c5cfc, #00f5c4)", color: "#08111f", fontWeight: 800, cursor: "pointer" },
};
