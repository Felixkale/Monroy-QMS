...
function canonicalizeFieldKey(sectionKey, key) {
  ...
  const bySection = {
    boom: {
      max_working_height:"max_height", max_working_height_m:"max_height",
      working_height:"max_height", working_height_m:"max_height", height:"max_height",
      boom_length_min:"min_boom_length", min_boom:"min_boom_length", min_length:"min_boom_length",
      minimum_boom_length:"min_boom_length", boom_length_max:"max_boom_length",
      max_boom:"max_boom_length", max_length:"max_boom_length",
      maximum_boom_length:"max_boom_length", boom_length_actual:"actual_boom_length",
      actual_boom:"actual_boom_length", actual_length:"actual_boom_length",
      actual_test_config:"actual_boom_length", extended_telescoped:"extended_boom_length",
      telescoped:"extended_boom_length", extended:"extended_boom_length",
      telescoped_length:"extended_boom_length", angle:"boom_angle", boom_angle_deg:"boom_angle",
      radius_min:"min_radius", minimum_radius:"min_radius", radius_max:"max_radius",
      maximum_radius:"max_radius", actual_radius:"load_tested_at_radius",
      test_radius:"load_tested_at_radius", working_radius_tested:"load_tested_at_radius",
      actual_working_radius:"load_tested_at_radius", swl_min_radius:"swl_at_min_radius",
      min_radius_swl:"swl_at_min_radius", swl_max_radius:"swl_at_max_radius",
      max_radius_swl:"swl_at_max_radius", swl_actual:"swl_at_actual_config",
      actual_swl:"swl_at_actual_config", actual_config_swl:"swl_at_actual_config",
      swl_at_radius:"swl_at_actual_config", load_test:"test_load",
      test_load_applied:"test_load", load_test_applied:"test_load", proof_load:"test_load",
      structure:"boom_structure", boom_condition:"boom_structure",
      pins_connections:"boom_pins", pins:"boom_pins", wear_pads:"boom_wear", pads:"boom_wear",
      extension_system:"luffing_system", luffing:"luffing_system",
      rotation_system:"slew_system", slew_rotation:"slew_system",
      hoist_lift:"hoist_system", lift_system:"hoist_system",
      lmi_tested:"lmi_test", lmi_tested_at_config:"lmi_test",
      anti_two_block_overload:"anti_two_block", overload_system:"anti_two_block",
    ...
    bucket: {
      ...
      gate_latch_system:"gate_latch", gate:"gate_latch", mounting:"platform_mounting",
      attachment_to_boom:"platform_mounting", slew:"rotation", anchor_points:"harness_anchors",
      swl_marking_legible:"swl_marking", coating_condition:"paint_condition",
      auto_levelling:"levelling_system", emergency_lowering_device:"emergency_lowering",
      emergency_stop_platform:"emergency_stop", overload_swl_cut_off_device:"overload_device",
      inclination_alarm:"tilt_alarm", communication:"intercom",
    },
    checklist: {
      structural_integrity:"structural_result", structure:"structural_result",
      hydraulic_system:"hydraulics_result", hydraulics:"hydraulics_result",
      brakes:"brakes_result", brake_drive_system:"brakes_result",
      tyres_wheels:"tyres_result", tires_wheels:"tyres_result",
      lights_alarms:"lights_result", safety_systems:"safety_devices",
...
function flattenNotesJson(raw) {
  ...
  function addRow(section, key, value, path) {
    ...
    const resolvedKey = sectionKey === "general"
      ? normalizeFieldToken(key)
      : canonicalizeFieldKey(sectionKey, key);
    rows.push({
      id: rowId++,
      section,
      key: resolvedKey || key,
      label: formatFieldLabel(resolvedKey || key),
      ...
      topVal.forEach((item, idx) => {
        if (typeof item === "object" && item !== null) {
          ...
          Object.entries(item).forEach(([k, v]) => {
            if (typeof v !== "object") addRow(secLabel, k, v, [topKey, idx, k]);
          });
        } else {
      ...
      Object.entries(topVal).forEach(([k, v]) => {
        if (v !== null && typeof v === "object" && !Array.isArray(v)) {
          ...
          Object.entries(v).forEach(([k2, v2]) => {
            if (typeof v2 !== "object") addRow(subSec, k2, v2, [topKey, k, k2]);
          });
...
function rebuildNotesJson(rows) {
  ...
  rows.forEach(({ path, value }) => {
    ...
    if (path.length === 1) {
      ...
      const sk = normalizeSectionPathKey(path[0]);
      const fk = canonicalizeFieldKey(sk, path[1]);
      if (!out[sk] || typeof out[sk] !== "object") out[sk] = {};
      out[sk][fk] = v;
    } else if (path.length === 3) {
      if (typeof path[1] === "number") {
        const ak = normalizeSectionPathKey(path[0]);
        const fk = canonicalizeFieldKey(ak, path[2]);
        ...
        const fk = canonicalizeFieldKey(sk, path[2]);
        if (!out[sk]) out[sk] = {};
        if (!out[sk][subKey] || typeof out[sk][subKey] !== "object") out[sk][subKey] = {};
        out[sk][subKey][fk] = v;
      }
    }
  });
...
function buildCherryPickerNotes(cp) {
  ...
  const boomKeys = [
    "max_height","min_boom_length","max_boom_length","actual_boom_length",
    "extended_boom_length","boom_angle","min_radius","max_radius",
    "load_tested_at_radius","swl_at_min_radius","swl_at_max_radius",
    "swl_at_actual_config","test_load","boom_structure","boom_pins","boom_wear",
    "luffing_system","slew_system","hoist_system","lmi_test","anti_two_block",
  ];
  boomKeys.forEach(k => { if (cp[k]) boom[k] = cp[k]; });
  ...
  const bucketMap = {
    ...
    harness_anchors:"harness_anchors", swl_marking:"swl_marking",
    paint_condition:"paint_condition", levelling_system:"levelling_system",
    emergency_lowering:"emergency_lowering", emergency_stop:"emergency_stop",
    overload_device:"overload_device", tilt_alarm:"tilt_alarm", intercom:"intercom",
  };
  Object.entries(bucketMap).forEach(([cpk, bk]) => { if (cp[cpk]) bucket[bk] = cp[cpk]; });
  if (cp.bucket_notes) bucket.notes = cp.bucket_notes;

  const clMap = {
    structural_result:"structural_result", hydraulics_result:"hydraulics_result",
    safety_devices:"safety_devices", fire_extinguisher:"fire_extinguisher",
    cl_emergency_stop:"emergency_stop", stabiliser_interlocks:"stabiliser_interlocks",
    machine_stable_under_load:"machine_stable_under_load",
    no_structural_deformation_under_load:"no_structural_deformation_under_load",
...
function parseCherryPickerNotes(notesStr) {
  const def = {
    ...
    extended_boom_length:"",boom_angle:"",min_radius:"",max_radius:"",
    load_tested_at_radius:"",swl_at_min_radius:"",swl_at_max_radius:"",
    swl_at_actual_config:"",test_load:"",
    ...
    paint_condition:"Satisfactory",levelling_system:"PASS",
    emergency_lowering:"PASS",emergency_stop:"PASS",overload_device:"PASS",
    tilt_alarm:"PASS",intercom:"PASS",bucket_notes:"",
    structural_result:"PASS",hydraulics_result:"PASS",safety_devices:"PASS",
    fire_extinguisher:"PASS",cl_emergency_stop:"PASS",stabiliser_interlocks:"PASS",
    machine_stable_under_load:"PASS",no_structural_deformation_under_load:"PASS",
    all_functions_operate_under_load:"PASS",
  };
  if (!notesStr) return def;
  let parsed = {};
  try { parsed = JSON.parse(notesStr); } catch(e) { return def; }
  const bm = parsed.boom || {};
  const bk = parsed.bucket || {};
  ...
  return {
    max_height:            g(bm,"max_height","max_working_height","working_height","height"),
    min_boom_length:       g(bm,"min_boom_length","boom_length_min","min_boom"),
    max_boom_length:       g(bm,"max_boom_length","boom_length_max","max_boom","boom_length"),
    actual_boom_length:    g(bm,"actual_boom_length","boom_length_actual","actual_boom"),
    extended_boom_length:  g(bm,"extended_boom_length","extended","telescoped"),
    boom_angle:            g(bm,"boom_angle","angle"),
    min_radius:            g(bm,"min_radius","radius_min"),
    max_radius:            g(bm,"max_radius","radius_max","working_radius"),
    load_tested_at_radius: g(bm,"load_tested_at_radius","actual_radius","test_radius"),
    swl_at_min_radius:     g(bm,"swl_at_min_radius","swl_min_radius"),
    swl_at_max_radius:     g(bm,"swl_at_max_radius","swl_max_radius"),
    swl_at_actual_config:  g(bm,"swl_at_actual_config","swl_actual","actual_swl","swl"),
    test_load:             g(bm,"test_load","test_load_applied","load_test"),
    boom_structure:        g(bm,"boom_structure","structure") || "PASS",
    boom_pins:             g(bm,"boom_pins","pins") || "PASS",
    boom_wear:             g(bm,"boom_wear","wear_pads") || "PASS",
    luffing_system:        g(bm,"luffing_system","extension_system") || "PASS",
    slew_system:           g(bm,"slew_system","rotation_system") || "PASS",
    hoist_system:          g(bm,"hoist_system","hoist_lift") || "PASS",
    lmi_test:              g(bm,"lmi_test","lmi_tested") || "PASS",
    anti_two_block:        g(bm,"anti_two_block","anti_two_block_overload") || "PASS",
    boom_notes:            g(bm,"notes","comments","remarks"),
    ...
    guardrails:            g(bk,"guardrails") || "PASS",
    toe_boards:            g(bk,"toe_boards","toe_board") || "PASS",
    gate_latch:            g(bk,"gate_latch","gate") || "PASS",
    platform_mounting:     g(bk,"platform_mounting","mounting") || "PASS",
    rotation:              g(bk,"rotation","slew") || "PASS",
    harness_anchors:       g(bk,"harness_anchors","anchor_points") || "PASS",
    swl_marking:           g(bk,"swl_marking","swl_marking_legible") || "PASS",
    paint_condition:       g(bk,"paint_condition","coating_condition") || "Satisfactory",
    levelling_system:      g(bk,"levelling_system","auto_levelling") || "PASS",
    emergency_lowering:    g(bk,"emergency_lowering","emergency_lowering_device") || "PASS",
    emergency_stop:        g(bk,"emergency_stop","emergency_stop_platform") || "PASS",
    overload_device:       g(bk,"overload_device","overload_swl_cut_off_device") || "PASS",
    tilt_alarm:            g(bk,"tilt_alarm","inclination_alarm") || "PASS",
    intercom:              g(bk,"intercom","communication") || "PASS",
    bucket_notes:          g(bk,"notes","comments"),
    structural_result:                    g(cl,"structural_result","structural_integrity") || "PASS",
    hydraulics_result:                    g(cl,"hydraulics_result","hydraulic_system") || "PASS",
    safety_devices:                       g(cl,"safety_devices","safety_systems") || "PASS",
    fire_extinguisher:                    g(cl,"fire_extinguisher") || "PASS",
    cl_emergency_stop:                    g(cl,"emergency_stop") || "PASS",
    stabiliser_interlocks:                g(cl,"stabiliser_interlocks","outrigger_interlocks") || "PASS",
    machine_stable_under_load:            g(cl,"machine_stable_under_load","machine_stable") || "PASS",
    no_structural_deformation_under_load: g(cl,"no_structural_deformation_under_load") || "PASS",
    all_functions_operate_under_load:     g(cl,"all_functions_operate_under_load") || "PASS",
  };
}
      ...
      {options.map(rv => {
        ...
        return (
          <button key={rv} type="button" onClick={() => onChange(rv)}
            style={{ padding:"6px 13px", borderRadius:8, border:`1px solid ${isActive ? col : T.border}`,
              background:isActive ? `${col}22` : T.card, color:isActive ? col : T.textDim,
...
function PFChip({ label, value, onChange }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8,
      ...
      background:"rgba(18,30,50,0.50)", marginBottom:5 }}>
        ...
        {OPTIONS.map(opt => {
          ...
          return (
            <button key={opt} type="button" onClick={() => onChange(opt)}
              style={{ padding:"3px 8px", borderRadius:6, border:`1px solid ${isActive ? c : "transparent"}`,
                background:isActive ? `${c}22` : "transparent", color:isActive ? c : T.textDim,
                fontWeight:800, fontSize:9, cursor:"pointer", fontFamily:"inherit",
                WebkitTapHighlightColor:"transparent", minHeight:28, whiteSpace:"nowrap" }}>
...
const JsonInspectionRow = memo(function JsonInspectionRow({ row, rowIndex, onChange, onRemove }) {
  ...
  const dotColor =
    ...
    : upperValue === "REPAIR_REQUIRED" ? "#fbbf24" : null;
  return (
    <tr style={{ background: rowIndex % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)" }}>
      ...
      <td className="id-val">
        <input value={row.value} onChange={e => onChange(row.id, e.target.value)}
          spellCheck={false} autoComplete="off"
          style={{ color:valueColor(row.value), fontWeight:/PASS|FAIL|YES|NO|REPAIR/.test(upperValue)?800:500 }}
          placeholder="—"/>
      </td>
      <td className="id-del">
        <button type="button" onClick={() => onRemove(row.id)}
          style={{ width:28,height:28,borderRadius:6,border:`1px solid ${T.redBrd}`,background:"transparent",
            color:T.red,fontWeight:800,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",opacity:0.6 }}
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        <F label="Boom Notes">
          <textarea value={cp.boom_notes||""} onChange={e => set("boom_notes",e.target.value)}
            rows={2} placeholder="Any boom-related observations…"
            style={{ ...IS, minHeight:60, resize:"vertical" }}/>
        </F>
        <F label="Platform / Bucket Notes">
          <textarea value={cp.bucket_notes||""} onChange={e => set("bucket_notes",e.target.value)}
            rows={2} placeholder="Any platform-related observations…"
            style={{ ...IS, minHeight:60, resize:"vertical" }}/>
        </F>
      </div>
    </div>
...
function CertificateEditInner() {
  ...
  const addJsonRow = useCallback(() => {
    ...
    setJsonRows(prev => [...prev, {
      id: Date.now() + Math.random(), section,
      key: resolvedKey || key, label: formatFieldLabel(resolvedKey || key),
      value: addValue.trim(), path,
    }]);
  ...
  const groupedRows = useMemo(() => {
    ...
    const filtered = query
      ? jsonRows.filter(r => String(r.label||"").toLowerCase().includes(query) || String(r.value||"").toLowerCase().includes(query) || String(r.section||"").toLowerCase().includes(query))
      : jsonRows;
  ...
  const SaveBtn = () => (
    <button type="button" onClick={handleSave} disabled={saving}
      style={{ padding:"12px 28px",borderRadius:11,border:"none",
        background:saving?"rgba(255,255,255,0.06)":"linear-gradient(135deg,#34d399,#14b8a6)",
        color:saving?"rgba(240,246,255,0.4)":"#052e16",fontWeight:900,fontSize:14,
                    ...
                    <>
                      {jsonRows.length > 0 && (
                        <input value={dataSearch} onChange={e => setDataSearch(e.target.value)}
                          placeholder="🔍  Search parameters or values…" style={{ ...IS,minHeight:40,fontSize:12 }}/>
                      )}
                      {jsonRows.length === 0 ? (
                        ...
                        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",gap:8,alignItems:"flex-end" }}>
                          <div>
                            ...
                            <input value={addSection} onChange={e => setAddSection(e.target.value)}
                              list="section-suggestions" placeholder="e.g. Checklist, Boom…"
                              style={{ ...IS,minHeight:40,fontSize:12 }}/>
                          ...
                          <div>
                            ...
                            <input value={addKey} onChange={e => setAddKey(e.target.value)}
                              placeholder="e.g. boom_length" style={{ ...IS,minHeight:40,fontSize:12 }}/>
                          </div>
                          <div>
                            <label style={{ ...LS,color:T.green }}>Value</label>
                            <input value={addValue} onChange={e => setAddValue(e.target.value)}
                              placeholder="e.g. PASS, 36m…" style={{ ...IS,minHeight:40,fontSize:12 }}
