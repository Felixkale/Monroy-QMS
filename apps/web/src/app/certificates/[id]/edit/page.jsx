 ...
 * across all fields where a result might live.
 */
...
export default function CertificateDetailsPage() {
  ...
  useEffect(() => {
    if (!id) return;
    if (searchParams.get("download") === "1") window.location.replace(`/certificates/print/${id}`);
  }, [searchParams, id]);
  ...
  async function loadCertificate() {
    ...
    if (data.folder_id) {
      const { data: linked, error: le } = await supabase.from("certificates").select("*")
        .eq("folder_id", data.folder_id).order("folder_position", { ascending: true }).order("created_at", { ascending: true });
  ...
  /* ✅ Use pickResult so "UNKNOWN" stored value doesn't block equipment_status */
      ...
      <div className="view-page-pad" style={{ minHeight:"100vh", background:`radial-gradient(ellipse 70% 50% at 0% 0%,rgba(34,211,238,0.05),transparent),${T.bg}`, color:T.text, fontFamily:"'IBM Plex Sans',sans-serif", padding:24 }}>
        <div style={{ maxWidth:1280, margin:"0 auto", display:"grid", gap:18 }}>
          ...
          {loading ? (
            ...
            <div style={{ background:"rgba(248,113,113,0.10)", border:`1px solid rgba(248,113,113,0.25)`, borderRadius:18, padding:18, color:"#f87171", fontSize:14, fontWeight:700 }}>⚠ {error}</div>
          ) : (
            <>
                ...
                <div style={{ background:T.panel, border:`1px solid ${T.border}`, borderRadius:18, padding:20 }}>
                  ...
                  <div style={{ display:"grid", gap:10 }}>
                    {bundle.map(item => {
                      ...
                      /* ✅ Each bundle item also uses pickResult */
