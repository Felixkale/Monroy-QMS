...
const T = {
  ...
  purple:"#a78bfa",purpleDim:"rgba(167,139,250,0.10)",purpleBrd:"rgba(167,139,250,0.25)",// src/app/certificates/[id]/page.jsx
"use client";
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box}
  ...
  @media(max-width:768px){
    .view-page-pad{padding:14px!important}
...
// ── Inner component isolates useSearchParams so Suspense can wrap it ──
function CertificateDetailsInner() {
  ...
  async function loadCertificate() {
    ...
    if (e || !data) {
      setRecord(null); setBundle([]);
      setError(e?.message || "Certificate not found.");
      setLoading(false); return;
    }
    setRecord(data);
    if (data.folder_id) {
      const { data: linked, error: le } = await supabase
          ...
          <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:20, padding:"20px 22px", backdropFilter:"blur(20px)" }}>
            <div className="view-hdr">
                ...
                <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                  ...
                  {record?.folder_id && (
                    <span style={{ padding:"5px 12px", borderRadius:99, background:T.accentDim, color:T.accent, border:`1px solid ${T.accentBrd}`, fontSize:11, fontWeight:800 }}>
                      📁 {record.folder_name || "Linked Folder"}
                    </span>
                  )}
                  {record?.equipment_type && (
                    <span style={{ padding:"5px 12px", borderRadius:99, background:T.card, color:T.textMid, border:`1px solid ${T.border}`, fontSize:11, fontWeight:700 }}>
                      {record.equipment_type}
                    </span>
                  )}
                </div>
              ...
              <div className="view-btn-row">
                ...
                <Link href={`/certificates/${id}/edit`} style={S.btnAmber}>Edit / Staple</Link>
                <button type="button" onClick={() => window.open(`/certificates/print/${id}`, "_blank", "noopener,noreferrer")} style={S.btnGreen}>
...
const S = {
  btnGhost:   { padding:"10px 16px", borderRadius:12, border:`1px solid rgba(148,163,184,0.18)`, background:"rgba(255,255,255,0.04)", color:"#f0f6ff", fontWeight:700, fontSize:13, cursor:"pointer", textDecoration:"none", display:"inline-flex", alignItems:"center", justifyContent:"center", fontFamily:"'IBM Plex Sans',sans-serif" },
  ...
  btnGreen:   { padding:"10px 16px", borderRadius:12, border:"none", background:"linear-gradient(135deg,#34d399,#14b8a6)", color:"#052e16", fontWeight:900, fontSize:13, cursor:"pointer", display:"inline-flex", alignItems:"center", justifyContent:"center", fontFamily:"'IBM Plex Sans',sans-serif" },
  btnGhostSm: { padding:"7px 12px", borderRadius:9, border:`1px solid rgba(148,163,184,0.18)`, background:"rgba(255,255,255,0.04)", color:"#f0f6ff", fontWeight:700, fontSize:12, textDecoration:"none", display:"inline-flex", alignItems:"center" },
};
};
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box}
  ...
  @media(max-width:480px){
    .view-cert-title{font-size:20px!important}
    .view-btn-row a,.view-btn-row button{font-size:11px!important;padding:9px 10px!important}
  }
`;
...
function resultTone(v){
  ...
  if(r==="PASS")            return{color:T.green, bg:T.greenDim, brd:T.greenBrd, label:"Pass"};
  if(r==="FAIL")            return{color:T.red,   bg:T.redDim,   brd:T.redBrd,   label:"Fail"};
  if(r==="REPAIR_REQUIRED") return{color:T.amber, bg:T.amberDim, brd:T.amberBrd, label:"Repair Required"};
  if(r==="OUT_OF_SERVICE")  return{color:T.purple,bg:T.purpleDim,brd:T.purpleBrd,label:"Out of Service"};
  return{color:T.textMid,bg:T.card,brd:T.border,label:"Unknown"};
}
...
export default function CertificateDetailsPage() {
  ...
  useEffect(()=>{if(id) loadCertificate();},[id]);
  useEffect(()=>{if(!id) return; if(searchParams.get("download")==="1") window.location.replace(`/certificates/print/${id}`);},[searchParams,id]);
  ...
  async function loadCertificate(){
    ...
    setRecord(data);
    if(data.folder_id){
      const{data:linked,error:le}=await supabase.from("certificates").select("*").eq("folder_id",data.folder_id).order("folder_position",{ascending:true}).order("created_at",{ascending:true});
      setBundle(le||!linked?.length?[data]:linked);
    }else{setBundle([data]);}
    setLoading(false);
  }

  const tone=useMemo(()=>resultTone(record?.result||record?.equipment_status),[record]);
        ...
        <div style={{maxWidth:1280,margin:"0 auto",display:"grid",gap:18}}>
          ...
          <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:20,padding:"20px 22px",backdropFilter:"blur(20px)"}}>
            <div className="view-hdr">
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:10,fontWeight:800,letterSpacing:"0.14em",textTransform:"uppercase",color:T.accent,marginBottom:8}}>Certificate Viewer</div>
                <h1 className="view-cert-title" style={{margin:"0 0 10px",fontSize:26,fontWeight:900,letterSpacing:"-0.02em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"100%"}}>
                  {record?.certificate_number||"Certificate"}
                </h1>
                <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                  <span style={{padding:"5px 12px",borderRadius:99,background:tone.bg,color:tone.color,border:`1px solid ${tone.brd}`,fontSize:11,fontWeight:800}}>{tone.label}</span>
                  {record?.folder_id&&<span style={{padding:"5px 12px",borderRadius:99,background:T.accentDim,color:T.accent,border:`1px solid ${T.accentBrd}`,fontSize:11,fontWeight:800}}>📁 {record.folder_name||"Linked Folder"}</span>}
                  {record?.equipment_type&&<span style={{padding:"5px 12px",borderRadius:99,background:T.card,color:T.textMid,border:`1px solid ${T.border}`,fontSize:11,fontWeight:700}}>{record.equipment_type}</span>}
                </div>
              </div>
              <div className="view-btn-row">
                <button type="button" onClick={()=>router.push("/certificates")} style={S.btnGhost}>← Back</button>
          ...
          {loading?(
            ...
            <>
              {/* Certificate sheet — dynamic, uses CertificateSheet component */}
              <CertificateSheet certificate={record} index={0} total={bundle.length||1}/>
              ...
              {bundle.length>1&&(
                <div style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:18,padding:20}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
                    <div style={{width:4,height:20,borderRadius:2,background:`linear-gradient(to bottom,${T.accent},rgba(34,211,238,0.3))`}}/>
                    <div style={{fontSize:16,fontWeight:900}}>Linked certificates in this folder</div>
                    <span style={{fontSize:11,fontWeight:700,padding:"3px 8px",borderRadius:99,background:T.accentDim,color:T.accent,border:`1px solid ${T.accentBrd}`}}>{bundle.length}</span>
                  </div>
                  <div style={{display:"grid",gap:10}}>
                    {bundle.map(item=>{
                      ...
                      return(
                        <div key={item.id} className="bundle-item" style={{border:`1px solid ${active?T.accentBrd:T.border}`,background:active?T.accentDim:T.card,borderRadius:14,padding:"13px 16px"}}>
                          <div>
                            <div style={{fontSize:14,fontWeight:800,color:T.text,marginBottom:3}}>{item.certificate_number||"—"}</div>
                            <div style={{color:T.textMid,fontSize:13}}>{item.equipment_description||item.asset_name||"Unnamed equipment"}</div>
                            <div style={{color:T.textDim,fontSize:11,marginTop:4}}>Position {item.folder_position||1}</div>
                          </div>
                          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
...
const S={
  btnGhost:{padding:"10px 16px",borderRadius:12,border:`1px solid rgba(148,163,184,0.18)`,background:"rgba(255,255,255,0.04)",color:"#f0f6ff",fontWeight:700,fontSize:13,cursor:"pointer",textDecoration:"none",display:"inline-flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Sans',sans-serif"},
  btnAmber:{padding:"10px 16px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#fbbf24,#f97316)",color:"#1a0a00",fontWeight:900,fontSize:13,textDecoration:"none",display:"inline-flex",alignItems:"center",justifyContent:"center"},
  btnGreen:{padding:"10px 16px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#34d399,#14b8a6)",color:"#052e16",fontWeight:900,fontSize:13,cursor:"pointer",textDecoration:"none",display:"inline-flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Sans',sans-serif"},
  btnGhostSm:{padding:"8px 12px",borderRadius:10,border:`1px solid rgba(148,163,184,0.18)`,background:"rgba(255,255,255,0.04)",color:"#f0f6ff",fontWeight:700,fontSize:12,textDecoration:"none",display:"inline-flex",alignItems:"center"},
};
