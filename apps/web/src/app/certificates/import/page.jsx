...
// ── Robust date normalizer — handles all formats AI might return ──────────
function normalizeDate(raw) {
  ...
  if (/^\d{1,2}\/\d{2}$/.test(s)) {
    ...
    return `${fy}-${String(m).padStart(2,"0")}-${String(last).padStart(2,"0")}`;
  }
  // DD/MM/YYYY
  if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(s)) {
    const [d, m, y] = s.split("/").map(Number);
...
export default function ImportCertificatesPage() {
  ...
  function clearOverrides() {
    setOverrides({ client_name: "", location: "", inspection_date: "", expiry_date: "" });
  }

  function setOverride(key, value) { setOverrides((prev) => ({ ...prev, [key]: value })); }

  // ── Override ALWAYS wins over extracted values ───────────────────────────
  ...
  async function handleExtract() {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        const item = files[i];
        setProgress(5 + (i / files.length) * 30, `Reading ${i + 1}/${files.length}: ${item.file.name}`);
        const b64 = await toBase64(item.file);
        payloads.push({ fileName: item.file.name, mimeType: item.file.type || "application/pdf", base64Data: b64 });
      }
      ...
      if (!Array.isArray(json?.results)) throw new Error("Unexpected response from /api/ai/extract");

      const mapped = json.results.map((item) => {
        if (!item.ok || !item.data) {
          return { ...item, saved: false, saving: false, saveError: null, expanded: false, certNumber: null, savedId: null, manualResult: "PASS", manualDefects: "" };
        }
        // Normalize dates BEFORE applying overrides
        const rawData = {
          ...item.data,
          inspection_date:     normalizeDate(item.data.inspection_date),
          expiry_date:         normalizeDate(item.data.expiry_date),
          next_inspection_due: normalizeDate(item.data.next_inspection_due),
        };
        const data = applyOverridesToData(rawData);
  ...
  function exportCsv() {
    const rows = results.filter((x) => x.ok && x.data).map((x) => ({ file_name: x.fileName, certificate_number: x.certNumber || "", result: x.manualResult, defects_found: x.manualDefects, ...x.data }));
    ...
    <AppLayout title="Import Certificates">
      <div className="cert-import-page">
        <div className="wrap">
          ...
          <div className="layout">
            <div className="left-col">
              ...
              <div className="card">
                <div className="card-header">
                  ...
                  <label className="browse-label">Browse<input ref={fileInputRef} type="file" multiple accept=".pdf,image/png,image/jpeg,image/webp" style={{display:"none"}} onChange={(e)=>addFiles(Array.from(e.target.files||[]))}/></label>
                </div>
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <div className="override-grid">
                    <div className="ov-f"><label className="ov-lbl">Client name</label><input className="ov-input" type="text" placeholder="e.g. Karowe Mine" value={overrides.client_name} onChange={(e)=>setOverride("client_name",e.target.value)}/></div>
              ...
              <div className="card">
                ...
                <div className="card-body" style={{paddingBottom:10}}>
                  {!files.length?<div className="empty-state">No files added yet.</div>:files.map((item)=>{
                    ...
                    return(<div className="q-item" key={item.id}><div className="q-icon">{ext}</div><div style={{flex:1,minWidth:0}}><div className="q-name" title={item.file.name}>{item.file.name}</div><div className="q-size">{fileSizeLabel(item.file)}</div></div><button className="btn-remove" type="button" onClick={()=>removeFile(item.id)}>✕</button></div>);
                  })}
                </div>
              </div>
            </div>
            ...
            <div className="card" style={{borderRadius:"var(--rxl)"}}>
              <div className="card-header">
                <div><div className="card-title">Extracted results</div><div className="card-sub">Review, set result & defects, then save to register</div></div>
                ...
                <div className="result-list">
                  {!results.length?(
                    <div className="empty-state" style={{padding:"32px 0"}}>
                      <svg width="36" height="36" viewBox="0 0 36 36" fill="none" style={{margin:"0 auto 12px",display:"block",opacity:0.3}}><rect x="6" y="4" width="24" height="28" rx="3" stroke="currentColor" strokeWidth="1.5"/><path d="M12 12h12M12 17h12M12 22h7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
                      ...
                      return(
                        <div key={`${item.fileName}-${idx}`} className={`rcard${item.ok?(item.saved?" is-saved":""):" is-err"}`}>
                          <div className="rhead">
                            ...
                            {item.ok?<span className="pill p-info">{filled} fields</span>:null}
                          ...
                          {!item.ok?(
                            <div className="rbody"><div className="err-box"><div className="err-title">{item.error||"Extraction failed."}</div><div className="err-detail">Check that /api/ai/extract is deployed and your AI API key is set.</div></div></div>
                          ):(
                            <>
                              <div className="rbody">
                                ...
                                <div className="two-fields">
                                  <div>
                                    ...
                                    <select className="sel" value={item.manualResult} disabled={disabled} onChange={(e)=>setResultField(idx,"manualResult",e.target.value)}>
                                      <option value="PASS">PASS</option><option value="FAIL">FAIL</option><option value="CONDITIONAL">CONDITIONAL</option><option value="UNKNOWN">UNKNOWN</option>
                                    </select>
                                  </div>
                                  <div>
                                ...
                                <div className="rfoot">
                                  ...
                                  <div className="foot-actions">
                                    {item.saved&&item.savedId?(<><Link href={`/certificates/${item.savedId}`} className="view-btn">View →</Link><Link href={`/certificates/${item.savedId}/edit`} className="edit-btn">Edit</Link></>):null}
                              ...
                              {item.expanded?(
                                <div className="drawer"><div className="drawer-grid">
                                  {Object.entries(d).map(([key,value])=>(
                                    <div className="dc" key={key}><div className="dc-k">{key.replace(/_/g," ")}</div><div className="dc-v">{value!=null&&String(value).trim()!==""?String(value):"—"}</div></div>
                                  ))}
                                </div></div>
                              ):null}
                            </>
        ...
        <style jsx global>{`
          ...
          @keyframes pulse{0%,100%{opacity:1;box-shadow:0 0 0 0 rgba(0,212,255,0.4)}50%{opacity:.8;box-shadow:0 0 0 6px rgba(0,212,255,0)}}
          ...
          .cert-import-page .stat-lbl{font-size:11px;color:var(--sub);font-weight:500;letter-spacing:.06em;text-transform:uppercase;margin-bottom:8px}
          .cert-import-page .stat-val{font-size:28px;font-weight:700;letter-spacing:-.03em}
          ...
          .cert-import-page .ov-input{padding:8px 10px;border-radius:var(--r);border:1px solid var(--b1);background:var(--s2);color:var(--text);font-size:12px;outline:none;font-family:inherit;width:100%;min-height:36px}
          .cert-import-page .ov-input:focus{border-color:var(--blue)}
          .cert-import-page .abadge{display:inline-flex;align-items:center;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;background:var(--blue-dim);color:var(--blue-t);border:1px solid #1a3a6a;letter-spacing:.04em}
          ...
          .cert-import-page .q-icon{width:32px;height:32px;border-radius:7px;background:var(--blue-dim);color:var(--blue-t);display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;flex-shrink:0;letter-spacing:.04em}
          ...
          .cert-import-page .pill{display:inline-flex;align-items:center;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;flex-shrink:0;letter-spacing:.04em}
          ...
          .cert-import-page .kv-lbl{font-size:10px;color:var(--hint);margin-bottom:3px;letter-spacing:.05em}
          ...
          .cert-import-page .view-btn{padding:6px 12px;border-radius:var(--r);border:1px solid var(--blue-dim);background:transparent;color:var(--blue-t);font-size:11px;cursor:pointer;font-family:inherit;text-decoration:none;display:inline-flex;align-items:center}
          ...
          .cert-import-page .dc-k{font-size:10px;color:var(--hint);margin-bottom:3px;text-transform:capitalize;letter-spacing:.03em}
