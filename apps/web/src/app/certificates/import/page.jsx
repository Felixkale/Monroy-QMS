...
function isAllowedFile(file) {
  return file && (file.type === "application/pdf" || file.type.startsWith("image/")) && file.size <= MAX_FILE_SIZE;
}
function nonEmpty(data) {
  return Object.values(data || {}).filter(v => v != null && String(v).trim() !== "").length;
}
...
function slugify(value) {
  return String(value || "").trim().toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 16) || "UNKNOWN";
}
...
export default function ImportCertificatesPage() {
  ...
  const [progress, setProgressState] = useState({ visible:false, pct:0, label:"Processing..." });
  ...
  function setProgress(pct, label) {
    setProgressState({ visible:true, pct:Math.round(pct), label:label || (pct < 100 ? "Processing..." : "Extraction complete") });
  }
  ...
  function addFiles(list) {
    setFiles(prev => {
      const next = [...prev];
      list.filter(isAllowedFile).forEach(file => {
        const exists = next.find(x => x.file.name === file.name && x.file.size === file.size);
        if (!exists && next.length < MAX_FILES) next.push({ id:uid(), file });
      });
      return next;
    });
    resetFileInputs();
  ...
  function setOverride(key, value) { setOverrides(prev => ({ ...prev, [key]:value })); }
  ...
  function applyOverridesToData(data) {
    ...
    // always hardcode inspector
    next.inspector_name = INSPECTOR_NAME;
    next.inspector_id   = INSPECTOR_ID;
    return next;
  }
  ...
  function setResultField(idx, key, value) {
    setResults(prev => prev.map((item, i) => i === idx ? { ...item, [key]:value } : item));
  }
  ...
  async function saveOne(idx) {
    ...
    setResults(prev => prev.map((item, i) => i === idx ? { ...item, saving:true, saveError:null } : item));
    try {
      const certNumber = genCert(row.data, row.fileName);
      const payload = {
        certificate_number:    certNumber,
        certificate_type:      row.data.certificate_type     || "Load Test Certificate",
        inspection_number:     row.data.inspection_number    || null,
        inspection_no:         row.data.inspection_number    || null,
        result:                row.manualResult || row.data.result || "UNKNOWN",
        equipment_status:      row.manualResult || row.data.result || "UNKNOWN",
        issue_date:            row.data.inspection_date      || row.data.issue_date || null,
        issued_at:             row.data.inspection_date ? new Date(row.data.inspection_date).toISOString() : null,
        expiry_date:           row.data.expiry_date          || row.data.next_inspection_due || null,
        valid_to:              row.data.expiry_date          || row.data.next_inspection_due || null,
        next_inspection_date:  row.data.next_inspection_due  || row.data.expiry_date || null,
        ...
        company:               row.data.client_name          || null,
        status:                "active",
        ...
        capacity:              row.data.capacity_volume      || null,
        capacity_volume:       row.data.capacity_volume      || null,
        swl:                   row.data.swl                  || null,
        mawp:                  row.data.working_pressure     || null,
        ...
        equipment_location:    row.data.location             || null,
        ...
        comments:              row.manualComments || row.data.comments || null,
        remarks:               row.manualComments || row.data.comments || row.manualDefects || null,
        logo_url:              "/logo.png",
      ...
      setResults(prev => prev.map((item, i) => i === idx ? { ...item, saving:false, saved:true, certNumber, savedId, saveError:null } : item));
    } catch(e) {
      setResults(prev => prev.map((item, i) => i === idx ? { ...item, saving:false, saved:false, saveError:e.message||"Save failed." } : item));
    }
  }
  ...
  function exportCsv() {
    const rows = results.filter(x => x.ok && x.data).map(x => ({ file_name:x.fileName, certificate_number:x.certNumber||"", result:x.manualResult, defects_found:x.manualDefects, comments:x.manualComments, ...x.data }));
    if (!rows.length) return;
    const headers = [...new Set(rows.flatMap(r => Object.keys(r)))];
    const csv = [headers.join(","), ...rows.map(row => headers.map(h => escapeCsv(row[h])).join(","))].join("\n");
    const blob = new Blob([csv], { type:"text/csv;charset=utf-8;" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "certificates.csv"; a.click(); URL.revokeObjectURL(a.href);
  }
    ...
    <AppLayout title="Import Certificates">
      <div className="cert-import-page">
            ...
            <div className="left-col">
              ...
              <div className="card">
                <div className="card-header">
                  <div>
                    <div className="card-title">
                      <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M8 2v8M4 7l4-5 4 5M2 14h12" stroke="var(--blue-t)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      Upload zone
                    </div>
                    <div className="card-sub">PDF · PNG · JPG · WEBP — max 20 files, 10 MB each</div>
                  </div>
                  <label className="browse-label">
                    Browse
                    <input ref={fileInputRef} type="file" multiple accept=".pdf,image/png,image/jpeg,image/webp" style={{display:"none"}} onChange={e=>addFiles(Array.from(e.target.files||[]))}/>
                  </label>
                </div>
                <div className="card-body">
                  ...
                  >
                    <input ref={dropInputRef} type="file" multiple accept=".pdf,image/png,image/jpeg,image/webp" onChange={e=>addFiles(Array.from(e.target.files||[]))}/>
                    <div className="drop-icon-ring">
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 3v13M6 9l6-6 6 6" stroke="var(--accent)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/><path d="M3 20h18" stroke="var(--accent)" strokeWidth="1.6" strokeLinecap="round"/></svg>
                    </div>
                    <div className="drop-h">Drop files here</div>
                    <div className="drop-p">Certificates, nameplates, equipment photos</div>
                    <div className="type-chips">
                      {["PDF","PNG","JPG","WEBP"].map(t=><span key={t} className="chip">{t}</span>)}
                    </div>
                  </div>
                  <div className="action-row">
                    ...
                    <button className="btn btn-primary" type="button" onClick={handleExtract} disabled={!files.length||extracting}>
                      <svg width="13" height="13" viewBox="0 0 16 16" fill="none" style={{flexShrink:0}}><circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.4"/><path d="M8 5v3l2 2" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>
                      {extracting ? "Extracting..." : "Extract with AI"}
                    </button>
                  </div>
                  <div className="prog-wrap" style={{display:progress.visible?"":"none"}}>
                ...
                <div className="card-header">
                  <div>
                    <div className="card-title" style={{gap:8}}>
                      Manual override
                      {overrideCount ? <span className="abadge">{overrideCount} active</span> : null}
                    </div>
                    <div className="card-sub">Fills missing fields. Won&apos;t overwrite extracted values.</div>
                  </div>
                ...
                <div className="card-body">
                  <div className="override-grid">
                    <div className="ov-f">
                      ...
                      <input className="ov-input" type="text" placeholder="e.g. Karowe Mine" value={overrides.client_name} onChange={e=>setOverride("client_name",e.target.value)}/>
                    </div>
                    <div className="ov-f">
                      <label className="ov-lbl">Location / Site</label>
                      <input className="ov-input" type="text" placeholder="e.g. Processing Plant" value={overrides.location} onChange={e=>setOverride("location",e.target.value)}/>
                    </div>
                    <div className="ov-f">
                      <label className="ov-lbl">Inspection date</label>
                      <input className="ov-input" type="date" value={overrides.inspection_date} onChange={e=>setOverride("inspection_date",e.target.value)}/>
                    </div>
                    <div className="ov-f">
                      <label className="ov-lbl">Next inspection date</label>
                      <input className="ov-input" type="date" value={overrides.expiry_date} onChange={e=>setOverride("expiry_date",e.target.value)}/>
                    </div>
                  </div>
                  {/* Hardcoded inspector info */}
                  <div className="insp-strip">
                    <div className="insp-item"><div className="ov-lbl">Inspector</div><div className="insp-val">{INSPECTOR_NAME}</div></div>
                    <div className="insp-item"><div className="ov-lbl">Inspector ID</div><div className="insp-val mono">{INSPECTOR_ID}</div></div>
                  </div>
                </div>
              </div>
              ...
              <div className="card">
                <div className="card-header">
                  <div><div className="card-title">Queue</div><div className="card-sub">{files.length} / 20 selected</div></div>
                </div>
                    ...
                    files.map(item => {
                      ...
                      return (
                        <div className="q-item" key={item.id}>
                          ...
                          <button className="btn-remove" type="button" onClick={()=>removeFile(item.id)}>✕</button>
                        </div>
                      );
                    })
                  )}
            ...
            <div className="card" style={{borderRadius:"var(--rxl)"}}>
              <div className="card-header">
                <div>
                  ...
                  <div className="card-sub">Review, set result &amp; defects, then save to register</div>
                </div>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <button className="btn-csv" type="button" onClick={exportCsv}>↓ CSV</button>
                ...
                <div className="result-list">
                  {!results.length ? (
                    <div className="empty-state" style={{padding:"32px 0"}}>
                      <svg width="36" height="36" viewBox="0 0 36 36" fill="none" style={{margin:"0 auto 12px",display:"block",opacity:0.3}}>
                    ...
                    results.map((item, idx) => {
                      ...
                      return (
                        <div key={`${item.fileName}-${idx}`} className={`rcard${item.scanning?" is-scanning":item.ok?(item.saved?" is-saved":""):" is-err"}`}>
                          <div className="rhead">
                            <div className="rnum" style={item.scanning?{background:"var(--blue-dim)",color:"var(--blue-t)"}:item.ok?{background:"var(--green-bg)",color:"var(--green-t)"}:{background:"var(--red-bg)",color:"var(--red-t)"}}>{idx+1}</div>
                            <div className="rfname" title={item.fileName}>{item.fileName}</div>
                            {item.scanning ? <span className="pill p-info"><span className="spinner"/>Scanning…</span> : null}
                            {item.ok && !item.scanning ? <span className="pill p-info">{filled} fields</span> : null}
                            {item.ok && d.equipment_type ? <span className="pill p-neutral">{d.equipment_type}</span> : null}
                            {item.ok ? <span className={`pill ${pillClass(r)}`}>{r}</span> : null}
                            {item.saved && item.certNumber ? <span className="cert-num">{item.certNumber}</span> : null}
                            {!item.scanning ? <span className={`pill ${item.ok?"p-ok":"p-err"}`}>{item.ok?"OK":"Error"}</span> : null}
                          </div>

                          {item.scanning ? (
                            <div className="rbody" style={{color:"var(--sub)",fontSize:12}}>Extracting fields with Gemini…</div>
                          ) : !item.ok ? (
                            <div className="rbody">
                            ...
                            <>
                              <div className="rbody">
                                ...
                                {d.raw_text_summary ? <div className="raw-sum">{d.raw_text_summary}</div> : null}

                                {/* Comments — appears after extraction */}
                                <div style={{marginBottom:12}}>
                                  <label className="field-lbl">Comments</label>
                                  <textarea className="ta" value={item.manualComments} disabled={disabled} placeholder="Add comments for this certificate…" onChange={e=>setResultField(idx,"manualComments",e.target.value)}/>
                                </div>
                                ...
                                <div className="two-fields">
                                  ...
                                  <div>
                                    ...
                                    <textarea className="ta" value={item.manualDefects} disabled={disabled} placeholder="Describe defects, cracks, wear, non-conformances..." onChange={e=>setResultField(idx,"manualDefects",e.target.value)}/>
                                  </div>
                                </div>

                                {/* Hardcoded inspector display */}
                                <div className="insp-strip" style={{marginBottom:10}}>
                                  <div className="insp-item"><div className="ov-lbl">Inspector</div><div className="insp-val">{INSPECTOR_NAME}</div></div>
                                  <div className="insp-item"><div className="ov-lbl">Inspector ID</div><div className="insp-val mono">{INSPECTOR_ID}</div></div>
                                </div>

                                {item.saveError ? <div className="save-err">{item.saveError}</div> : null}
                                  ...
                                  <div className="foot-actions">
                                    {item.saved && item.savedId ? (
                                      <>
                                        ...
                                        <a href={`/certificates/print/${item.savedId}`} target="_blank" rel="noopener noreferrer" className="pdf-btn">⬇ Save as PDF</a>
                                      </>
                                    ) : null}
                                  ...
                                  <div className="drawer-grid">
                                    {Object.entries(d).map(([key,value])=>(
                                      <div className="dc" key={key}>
                                        <div className="dc-k">{key.replace(/_/g," ")}</div>
                                        <div className="dc-v">{value!=null&&String(value).trim()!==""?String(value):"—"}</div>
                                      </div>
                                    ))}
                                  </div>
                                </div>
        ...
        <style jsx global>{`
          ...
          .cert-import-page{
            --bg:#060c18;--s1:#0d1526;--s2:#111d30;--s3:#162038;
            --b1:#1a2740;--b2:#243450;--b3:#2e4060;
            --text:#eef2f8;--sub:#7a8fa8;--hint:#4a5f78;
            --blue:#4a90e2;--blue2:#2d6bc4;--blue-dim:#122040;--blue-t:#7eb8f7;
            --green:#22c55e;--green-bg:#0a2818;--green-b:#145228;--green-t:#4ade80;
            --red:#ef4444;--red-bg:#200a0a;--red-b:#5c1a1a;--red-t:#f87171;
            --amber:#f59e0b;--amber-bg:#1e1208;--amber-b:#6b3d08;--amber-t:#fbbf24;
          ...
          .cert-import-page .wrap{padding:28px 24px;max-width:1160px}
          .cert-import-page .top-bar{display:flex;align-items:center;justify-content:space-between;margin-bottom:32px;gap:16px;flex-wrap:wrap}
          .cert-import-page .brand{display:flex;align-items:center;gap:10px}
          ...
          .cert-import-page .brand-label{font-size:11px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;color:var(--sub)}
          .cert-import-page .brand-title{font-size:22px;font-weight:700;color:var(--text);letter-spacing:-0.02em;margin-top:2px}
          .cert-import-page .nav-btns{display:flex;gap:8px;flex-wrap:wrap}
          .cert-import-page .nav-btn{padding:8px 16px;border-radius:var(--r);border:1px solid var(--b2);background:var(--s1);color:var(--sub);font-size:12px;font-weight:500;cursor:pointer;font-family:inherit;text-decoration:none;display:inline-flex;align-items:center;gap:6px;transition:border-color .15s,color .15s}
          .cert-import-page .nav-btn:hover{border-color:var(--b3);color:var(--text)}
          .cert-import-page .nav-btn-primary{background:var(--blue2);border-color:var(--blue2);color:#fff}
          .cert-import-page .nav-btn-primary:hover{background:var(--blue);border-color:var(--blue)}
          .cert-import-page .stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:24px}
          .cert-import-page .stat-card{background:var(--s1);border:1px solid var(--b1);border-radius:var(--rl);padding:16px 18px;position:relative;overflow:hidden}
          ...
          .cert-import-page .stat-lbl{font-size:11px;color:var(--sub);font-weight:500;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:8px}
          .cert-import-page .stat-val{font-size:28px;font-weight:700;letter-spacing:-0.03em}
          ...
          .cert-import-page .layout{display:grid;grid-template-columns:360px 1fr;gap:16px;align-items:start}
          .cert-import-page .left-col{display:grid;gap:14px}
          .cert-import-page .card{background:var(--s1);border:1px solid var(--b1);border-radius:var(--rxl);overflow:hidden}
          .cert-import-page .card-header{padding:16px 20px;border-bottom:1px solid var(--b1);display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap}
          ...
          .cert-import-page .card-body{padding:16px 20px}
          .cert-import-page .browse-label{padding:8px 14px;border-radius:var(--r);background:var(--blue2);color:#fff;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit}
          .cert-import-page .drop-area{border:1.5px dashed var(--b2);border-radius:var(--rl);padding:28px 20px;text-align:center;background:var(--s2);cursor:pointer;position:relative;transition:border-color .2s,background .2s;margin-bottom:14px}
          .cert-import-page .drop-area:hover,.cert-import-page .drop-area.drag{border-color:var(--accent);background:rgba(0,212,255,0.04)}
          .cert-import-page .drop-area input{position:absolute;inset:0;opacity:0;cursor:pointer;width:100%;height:100%}
          .cert-import-page .drop-icon-ring{width:52px;height:52px;border-radius:50%;border:1.5px solid var(--b3);background:var(--s3);display:flex;align-items:center;justify-content:center;margin:0 auto 14px}
          .cert-import-page .drop-h{font-size:14px;font-weight:600;color:var(--text);margin-bottom:4px}
          .cert-import-page .drop-p{font-size:12px;color:var(--sub)}
          .cert-import-page .type-chips{display:flex;gap:5px;justify-content:center;margin-top:12px;flex-wrap:wrap}
          .cert-import-page .chip{font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;background:var(--s3);border:1px solid var(--b2);color:var(--sub);letter-spacing:0.06em}
          .cert-import-page .action-row{display:flex;gap:8px;align-items:center}
          .cert-import-page .btn{padding:9px 16px;border-radius:var(--r);border:1px solid var(--b2);background:var(--s2);color:var(--sub);cursor:pointer;font-size:12px;font-weight:500;font-family:inherit;transition:all .15s}
          .cert-import-page .btn:hover{border-color:var(--b3);color:var(--text)}
          .cert-import-page .btn-ghost{background:transparent;border-color:transparent;color:var(--hint)}
          .cert-import-page .btn-ghost:hover{color:var(--sub);border-color:transparent}
          ...
          .cert-import-page .btn-primary:hover:not(:disabled){background:var(--blue);border-color:var(--blue)}
          .cert-import-page .btn-primary:disabled{opacity:0.35;cursor:not-allowed}
          .cert-import-page .btn-save{padding:7px 14px;border-radius:var(--r);border:1px solid var(--green-b);background:var(--green-bg);color:var(--green-t);cursor:pointer;font-size:12px;font-weight:500;font-family:inherit;transition:opacity .15s;display:inline-flex;align-items:center;gap:5px}
          .cert-import-page .btn-save:disabled{opacity:0.4;cursor:not-allowed}
          ...
          .cert-import-page .btn-csv{padding:7px 13px;border-radius:var(--r);border:1px solid var(--b2);background:var(--s2);color:var(--sub);cursor:pointer;font-size:12px;font-family:inherit}
          .cert-import-page .btn-saveall{padding:9px 16px;border-radius:var(--r);border:none;background:var(--blue2);color:#fff;cursor:pointer;font-size:12px;font-weight:600;font-family:inherit}
          .cert-import-page .btn-saveall:disabled{opacity:0.35;cursor:not-allowed}
          ...
          .cert-import-page .override-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px}
          .cert-import-page .ov-f{display:flex;flex-direction:column;gap:4px}
          .cert-import-page .ov-lbl{font-size:10px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:var(--hint)}
          .cert-import-page .ov-input{padding:8px 10px;border-radius:var(--r);border:1px solid var(--b1);background:var(--s2);color:var(--text);font-size:12px;outline:none;font-family:inherit;width:100%;transition:border-color .15s}
          .cert-import-page .ov-input:focus{border-color:var(--blue)}
          .cert-import-page .insp-strip{display:flex;gap:16px;padding:8px 12px;background:rgba(0,212,255,0.04);border:1px solid rgba(0,212,255,0.12);border-radius:var(--r);margin-top:4px}
          .cert-import-page .insp-item{display:flex;flex-direction:column;gap:3px}
          .cert-import-page .insp-val{font-size:12px;font-weight:700;color:var(--text)}
          .cert-import-page .insp-val.mono{font-family:"IBM Plex Mono",monospace;font-size:11px}
          .cert-import-page .abadge{display:inline-flex;align-items:center;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;background:var(--blue-dim);color:var(--blue-t);border:1px solid #1a3a6a;letter-spacing:0.04em}
          ...
          .cert-import-page .q-icon{width:32px;height:32px;border-radius:7px;background:var(--blue-dim);color:var(--blue-t);display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;flex-shrink:0;letter-spacing:0.04em}
          ...
          .cert-import-page .rcard{border:1px solid var(--b1);border-radius:var(--rl);overflow:hidden;transition:border-color .2s}
          .cert-import-page .rcard.is-err{border-color:var(--red-b)}
          .cert-import-page .rcard.is-saved{border-color:var(--green-b)}
          .cert-import-page .rhead{display:flex;align-items:center;gap:8px;padding:11px 16px;background:var(--s2);flex-wrap:wrap;border-bottom:1px solid var(--b1)}
          ...
          .cert-import-page .pill{display:inline-flex;align-items:center;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;flex-shrink:0;letter-spacing:0.04em}
          ...
          .cert-import-page .rbody{padding:16px 18px}
          ...
          .cert-import-page .kv-lbl{font-size:10px;color:var(--hint);margin-bottom:3px;letter-spacing:0.05em}
          ...
          .cert-import-page .field-lbl{font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--hint);margin-bottom:5px;display:block}
          .cert-import-page .sel{padding:8px 10px;border-radius:var(--r);border:1px solid var(--b1);background:var(--s2);color:var(--text);font-size:12px;outline:none;width:100%;font-family:inherit;cursor:pointer}
          .cert-import-page .sel:disabled{opacity:0.45}
          .cert-import-page .sel:focus{border-color:var(--blue)}
          .cert-import-page .ta{padding:8px 10px;border-radius:var(--r);border:1px solid var(--b1);background:var(--s2);color:var(--text);font-size:12px;outline:none;width:100%;font-family:inherit;resize:vertical;min-height:64px;line-height:1.5}
          .cert-import-page .ta:disabled{opacity:0.45}
          .cert-import-page .ta:focus{border-color:var(--blue)}
          ...
          .cert-import-page .err-box{background:var(--red-bg);border:1px solid var(--red-b);border-radius:var(--r);padding:12px 14px;margin-bottom:10px}
          ...
          .cert-import-page .expand-btn:hover{color:var(--sub)}
          .cert-import-page .foot-actions{display:flex;gap:6px;align-items:center;flex-wrap:wrap}
          .cert-import-page .view-btn{padding:6px 12px;border-radius:var(--r);border:1px solid #1a3a6a;background:transparent;color:var(--blue-t);font-size:11px;cursor:pointer;font-family:inherit;text-decoration:none;display:inline-flex;align-items:center}
          .cert-import-page .edit-btn{padding:6px 12px;border-radius:var(--r);border:1px solid var(--amber-b);background:var(--amber-bg);color:var(--amber-t);font-size:11px;cursor:pointer;font-family:inherit;text-decoration:none;display:inline-flex;align-items:center}
          .cert-import-page .pdf-btn{padding:6px 12px;border-radius:var(--r);border:1px solid var(--green-b);background:var(--green-bg);color:var(--green-t);font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;text-decoration:none;display:inline-flex;align-items:center;gap:4px}
          .cert-import-page .pdf-btn:hover{opacity:0.85}
          .cert-import-page .drawer{border-top:1px solid var(--b1);background:var(--bg);padding:14px 18px}
          ...
          .cert-import-page .dc-k{font-size:10px;color:var(--hint);margin-bottom:3px;text-transform:capitalize;letter-spacing:0.03em}
          .cert-import-page .dc-v{font-size:11px;font-weight:600;color:var(--text);word-break:break-word;line-height:1.4}
          .cert-import-page .spinner{display:inline-block;width:11px;height:11px;border:2px solid var(--b3);border-top-color:var(--blue-t);border-radius:50%;animation:spin 0.6s linear infinite;vertical-align:middle;margin-right:4px}
          ...
          @media(max-width:780px){
            ...
            .cert-import-page .override-grid{grid-template-columns:1fr}
          }
        `}</style>
      </div>
    </AppLayout>
