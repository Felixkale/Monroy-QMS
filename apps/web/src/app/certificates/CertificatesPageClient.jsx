...
const CSS = `
  *,*::before,*::after{box-sizing:border-box}
  ...
  .equip-hdr{display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;padding:10px 14px;border-bottom:1px solid ${T.border}}
  .folder-badge{display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:6px;background:rgba(167,139,250,0.10);border:1px solid rgba(167,139,250,0.25);color:#a78bfa;font-size:10px;font-weight:700;white-space:nowrap}
  ...
  @media(max-width:768px){
    ...
    .equip-hdr{flex-direction:column;align-items:flex-start;gap:6px}
  }
...
export default function CertificatesPageClient() {
  ...
  // Restore persisted filter/view state on mount
  const ss = useMemo(() => loadSession(), []);
  const [search,   setSearch]   = useState(ss.search   || "");
  const [fResult,  setFResult]  = useState(ss.fResult  || "ALL");
  const [fExpiry,  setFExpiry]  = useState(ss.fExpiry  || "ALL");
  const [fClient,  setFClient]  = useState(ss.fClient  || "ALL");
  const [fType,    setFType]    = useState(ss.fType    || "ALL");
  const [fStatus,  setFStatus]  = useState(ss.fStatus  || "ALL");
  const [view,     setView]     = useState(ss.view     || "grouped");

  // Accordion state — restored from session
  const [openClients, setOpenClients] = useState(ss.openClients || {});
  const [openTypes,   setOpenTypes]   = useState(ss.openTypes   || {});
  ...
  useEffect(() => { saveSession({ search, fResult, fExpiry, fClient, fType, fStatus, view }); },
    [search, fResult, fExpiry, fClient, fType, fStatus, view]);

  useEffect(() => { saveSession({ openClients }); }, [openClients]);
  useEffect(() => { saveSession({ openTypes });   }, [openTypes]);
  ...
  async function loadCerts() {
    ...
    const { data, error } = await supabase.from("certificates").select(`
      ...
      inspection_number,inspection_no,inspection_date,asset_tag,asset_name,
      equipment_description,equipment_type,asset_type,client_name,company,
      status,folder_id,folder_name,folder_position,extracted_data,
    ...
    // On first load, open all clients — but preserve session state if it exists
    if (!ss.openClients || Object.keys(ss.openClients).length === 0) {
      const oc = {};
      cleaned.forEach(r => { oc[r.client_name] = true; });
      setOpenClients(oc);
    }
    setCerts(cleaned);
    setLoading(false);
    ...
    if (folderId) {
      ...
      if (remaining && remaining.length === 1)
        await supabase.from("certificates")
          .update({ folder_id: null, folder_name: null, folder_position: null }).eq("id", remaining[0].id);
    }
    await loadCerts();
  }
  ...
  const filtered = useMemo(() => {
    ...
    return certs.filter(r => {
      const hay = [r.certificate_number, r.client_name, r.asset_tag, r.asset_name,
        r.equipment_description, r.equipment_type, r.inspection_number, r.inspection_no,
        r.folder_name].filter(Boolean).join(" ").toLowerCase();
      return (!q || hay.includes(q))
        && (fResult === "ALL" || r.result          === fResult)
        && (fExpiry === "ALL" || r.expiry_bucket   === fExpiry)
  ...
  function clearFilters() {
    setSearch(""); setFResult("ALL"); setFExpiry("ALL"); setFClient("ALL"); setFType("ALL"); setFStatus("ALL");
  }

  function toggleClient(cl) {
    setOpenClients(p => ({ ...p, [cl]: !p[cl] }));
  }
  function toggleType(k) {
    setOpenTypes(p => ({ ...p, [k]: !p[k] }));
  }
        ...
        <div style={{ maxWidth:1500, margin:"0 auto", display:"grid", gap:16 }}>
          ...
          <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:20, padding:"18px 20px", backdropFilter:"blur(20px)" }}>
            <div className="cert-top" style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, marginBottom:14 }}>
              ...
              <div className="cert-top-btns" style={{ display:"flex", gap:8, flexShrink:0 }}>
                ...
                <Link href="/certificates/crane-inspection" style={S.ghost}>🏗 Crane</Link>
                <Link href="/certificates/machine-inspection" style={S.ghost}>🔧 Machine</Link>
              ...
              <div style={{ marginLeft:"auto", display:"flex", gap:8, flexWrap:"wrap" }}>
                {selectMode && selected.size >= 2 && (
                  <button type="button" onClick={linkSelected} disabled={linking}
                    style={{ padding:"8px 16px", borderRadius:9, border:"none", background:"linear-gradient(135deg,#a78bfa,#60a5fa)", color:"#fff", fontWeight:900, fontSize:13, cursor:"pointer", fontFamily:"'IBM Plex Sans',sans-serif" }}>
                    {linking ? "Linking…" : `🔗 Link ${selected.size} Certificates`}
                  </button>
                )}
                {selectMode && (
                  <button type="button" onClick={() => { setSelected(new Set()); setLinkMsg(""); }}
                    style={{ padding:"8px 14px", borderRadius:9, border:`1px solid ${T.border}`, background:T.card, color:T.textMid, fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"'IBM Plex Sans',sans-serif" }}>
                    Cancel
                  </button>
                )}
              </div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                {hasFilters && (
                  <button type="button" onClick={clearFilters}
                    style={{ background:"none", border:"none", color:T.textDim, fontSize:11, cursor:"pointer", fontWeight:700 }}>
                    Clear ×
                  </button>
                )}
                <div className="view-toggle" style={{ display:"flex", border:`1px solid ${T.border}`, borderRadius:8, overflow:"hidden" }}>
                  {["grouped","flat"].map(v => (
                    <button key={v} type="button" onClick={() => setView(v)}
                      style={{ border:"none", cursor:"pointer", padding:"5px 12px", fontSize:11, fontWeight:700,
          ...
          {loading ? (
            ...
            <div style={{ background:T.panel, border:`1px solid ${T.border}`, borderRadius:14, padding:"44px 20px", textAlign:"center" }}>
              ...
              {hasFilters && (
                <button type="button" onClick={clearFilters}
                  style={{ padding:"9px 18px", borderRadius:9, border:`1px solid ${T.accentBrd}`, background:T.accentDim, color:T.accent, fontWeight:800, fontSize:13, cursor:"pointer", fontFamily:"'IBM Plex Sans',sans-serif" }}>
                  Clear Filters
                </button>
              )}
            </div>
  ...
  return (
    <div style={{ display:"grid", gap:10 }}>
      {grouped.map(cg => {
        ...
        // Count unique folders in this client
        const folderIds = new Set(
          cg.types.flatMap(t => t.items.flatMap(i => i.certs.filter(c => c.folder_id).map(c => c.folder_id)))
        );
        ...
        return (
          <div key={cg.client} style={{ border:`1px solid ${T.border}`, borderRadius:14, overflow:"hidden", background:T.panel }}>
            ...
            <button type="button" onClick={() => toggleClient(cg.client)}
              style={{ width:"100%", border:"none", cursor:"pointer", textAlign:"left", padding:"14px 18px", background:"transparent", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, color:T.text, WebkitTapHighlightColor:"transparent" }}>
              <div style={{ display:"flex", alignItems:"center", gap:12, flex:1, minWidth:0 }}>
                <div style={{ width:36, height:36, borderRadius:10, background:T.accentDim, border:`1px solid ${T.accentBrd}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>🏢</div>
                <div style={{ minWidth:0 }}>
                  <div style={{ fontSize:15, fontWeight:900, color:T.text }}>{cg.client}</div>
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:3 }}>
                    <span style={{ fontSize:11, color:T.textDim }}>{count} cert{count === 1 ? "" : "s"}</span>
                    {folderIds.size > 0 && (
                      <span style={{ fontSize:11, color:T.purple }}>📁 {folderIds.size} folder{folderIds.size === 1 ? "" : "s"}</span>
                    )}
                    <span style={{ fontSize:11, color:T.textDim }}>{cg.types.length} equipment type{cg.types.length === 1 ? "" : "s"}</span>
                  </div>
                </div>
              </div>
            ...
            {open && (
              <div style={{ borderTop:`1px solid ${T.border}`, padding:12, display:"grid", gap:10 }}>
                {cg.types.map(tg => {
                  ...
                  const typeCount = tg.items.reduce((a, i) => a + i.certs.length, 0);
                  ...
                  return (
                    <div key={key} style={{ border:`1px solid ${T.border}`, borderRadius:11, overflow:"hidden", background:T.panel2 }}>
                      ...
                      <button type="button" onClick={() => toggleType(key)}
                        style={{ width:"100%", border:"none", cursor:"pointer", textAlign:"left", padding:"11px 14px", background:"transparent", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, color:T.text, WebkitTapHighlightColor:"transparent" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:10, flex:1, minWidth:0 }}>
                          <span style={{ fontSize:11, fontWeight:900, color:T.accent, background:T.accentDim, border:`1px solid ${T.accentBrd}`, borderRadius:6, padding:"2px 8px", whiteSpace:"nowrap" }}>
                            {tg.type}
                          </span>
                          <span style={{ fontSize:11, color:T.textDim }}>{typeCount} cert{typeCount === 1 ? "" : "s"}</span>
                        </div>
                      ...
                      {tOpen && (
                        <div style={{ padding:12, borderTop:`1px solid ${T.border}`, display:"grid", gap:10 }}>
                          {tg.items.map(item => {
                            // Gather inspection dates in this item group
                            const allDates = item.certs.map(c => c.inspection_date || c.issue_date).filter(Boolean).sort().reverse();
                            const latestDate  = allDates[0];
                            const earliestDate = allDates[allDates.length - 1];
                            const dateRange   = latestDate && earliestDate && latestDate !== earliestDate
                              ? `${formatDate(earliestDate)} – ${formatDate(latestDate)}`
                              : formatDate(latestDate);

                            // Count folders in this item
                            const itemFolders = new Set(item.certs.filter(c => c.folder_id).map(c => c.folder_id));
                            ...
                            return (
                              <div key={`${tg.type}-${item.desc}`} style={{ border:`1px solid ${T.border}`, borderRadius:10, overflow:"hidden", background:"rgba(255,255,255,0.015)" }}>
                                ...
                                {/* Equipment description header with inspection date */}
                                <div className="equip-hdr">
                                  <div style={{ minWidth:0 }}>
                                    <div style={{ fontSize:13, fontWeight:800, color:T.text }}>{item.desc}</div>
                                    {latestDate && (
                                      <div style={{ display:"flex", gap:8, marginTop:4, flexWrap:"wrap", alignItems:"center" }}>
                                        <span style={{ fontSize:10, color:T.textDim }}>🗓 Inspected: <span style={{ color:T.textMid, fontWeight:600 }}>{dateRange}</span></span>
                                        {itemFolders.size > 0 && (
                                          <span className="folder-badge">📁 {itemFolders.size} folder{itemFolders.size > 1 ? "s" : ""}</span>
                                        )}
                                      </div>
                                    )}
                                  </div>
                                  <span style={{ fontSize:10, color:T.textDim, whiteSpace:"nowrap" }}>{item.certs.length} cert{item.certs.length !== 1 ? "s" : ""}</span>
                                </div>
                                  ...
                                  <table style={{ width:"100%", borderCollapse:"collapse", minWidth:640 }}>
                                    <thead>
                                      <tr style={{ background:"rgba(255,255,255,0.02)" }}>
                                        {["Certificate No","Result","Inspected","Expiry","Status","Actions"].map(h => (
                                        ...
                                        if (cert._folderCerts && cert._folderCerts.length > 1) {
                                          return cert._folderCerts.map((fc, fi) => (
                                            <CertRow key={fc.id} cert={fc} compact allCerts={allCerts}
                                              ...
                                              folderRow={true} folderFirst={fi === 0}
                                              folderLast={fi === cert._folderCerts.length - 1}
                                              folderSize={cert._folderCerts.length}
    ...
    <div style={{ border:`1px solid ${T.border}`, borderRadius:14, overflow:"hidden", background:T.panel }}>
      <div className="cert-tbl" style={{ overflowX:"auto" }}>
        <table style={{ width:"100%", borderCollapse:"collapse", minWidth:900 }}>
          <thead>
            <tr style={{ background:"rgba(255,255,255,0.02)" }}>
              {["Certificate No","Client","Equipment","Type","Result","Inspected","Expiry","Days","Status","Actions"].map(h => (
...
// ── Cert Row ──────────────────────────────────────────────────────────────
function CertRow({ cert, compact=false, allCerts=[], onUnlink, onDelete, onSelect, selected, selectMode, folderRow=false, folderFirst=false, folderLast=false, folderSize=0, folderName="" }) {
  const r    = rc(cert.result);
  const e    = ec(cert.expiry_bucket);
  const days = daysDiff(cert.expiry_date);
  const id   = encodeURIComponent(String(cert.id ?? ""));

  if (compact) {
    return (
      <tr className="cr" style={{ borderBottom:`1px solid ${T.border}`, background: folderRow ? "rgba(167,139,250,0.03)" : "transparent" }}>
        <td style={{ ...TD, paddingLeft: folderRow ? 22 : 12 }}>
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            ...
            {folderFirst && folderName && (
              <span style={{ fontSize:9, color:T.purple, fontWeight:700 }}>{folderName}</span>
            )}
          </div>
        </td>
        ...
        <td style={TD}>
          <ActBtns id={id} certNo={cert.certificate_number} folderId={cert.folder_id} folderName={cert.folder_name} allCerts={allCerts} certId={cert.id} onUnlink={onUnlink} onDelete={onDelete} onSelect={onSelect} selected={selected?.has(id)} selectMode={selectMode}/>
        </td>
      </tr>
    );
  }
  ...
  return (
    <div style={{ padding:"13px 14px", borderBottom:`1px solid ${T.border}`, display:"grid", gap:9 }}>
      ...
      <div style={{ fontSize:11, color:T.textDim, display:"flex", gap:12, flexWrap:"wrap" }}>
        ...
        {cert.folder_name && <span style={{ color:T.purple }}>📁 {cert.folder_name}</span>}
      </div>
