...
function detectEquipmentType(assetType="") {
  ...
  return ["pressure vessel","boiler","air receiver","air compressor","oil separator"].includes(t) ? "pv" : "lift";
}

function defaultCertificateType(assetType="") {
  return detectEquipmentType(assetType) === "pv" ? "Pressure Test Certificate" : "Load Test Certificate";
}
...
export default function EditCertificatePage() {
  ...
  const [form, setForm] = useState({
    asset_id:"", certificate_type:"", company:"",
    equipment_description:"", equipment_location:"",
    equipment_id:"", lanyard_serial_no:"",          // ← new
    swl:"", mawp:"",
    equipment_status:"PASS", issued_at:"", valid_to:"",
    status:"issued",
    legal_framework:"Mines, Quarries, Works and Machinery Act Cap 44:02",
    inspector_name:"", inspector_id:"",
    signature_url:"", logo_url:"/logo.png", pdf_url:"",
  });
  ...
  useEffect(() => {
    supabase.from("assets").select(`
      ...
      serial_number, lanyard_serial_no, safe_working_load, working_pressure,
  ...
  useEffect(() => {
    ...
    supabase.from("certificates").select("*").eq("id",id).single()
      .then(({ data, error:err }) => {
        if (err||!data) { setError(err?.message||"Certificate not found."); return; }
        setForm({
          ...
          lanyard_serial_no:     data.lanyard_serial_no     || "",   // ← new
          swl:                   data.swl                   || "",
          mawp:                  data.mawp                  || "",
  ...
  function handleUseEquipmentData() {
    ...
    setForm(prev => ({
      ...
      lanyard_serial_no:     selectedAsset.lanyard_serial_no || prev.lanyard_serial_no,  // ← new
      // ── SWL stored as-is — no unit appended ──
  ...
  async function handleSubmit(e) {
    ...
    try {
      const { error:err } = await supabase.from("certificates").update({
        ...
        lanyard_serial_no:     form.lanyard_serial_no     || null,  // ← new
        swl:                   form.swl                   || null,  // ← stored as-is
        mawp:                  form.mawp                  || null,  // ← stored as-is
    ...
    <AppLayout title="Edit Certificate">
      <div style={{ maxWidth:1000 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
          ...
          {/* Right: print button */}
          <button onClick={() => window.open(`/certificates/print/${id}`,"_blank")}
            style={{ padding:"10px 20px", borderRadius:8,
              background:"linear-gradient(135deg,#00f5c4,#4fc3f7)",
              color:"#0d1117", border:"none", fontWeight:700, cursor:"pointer", fontSize:13 }}>
            🖨 Print Certificate
          </button>
        </div>
        ...
        <div style={{ width:60, height:3, borderRadius:999,
          background:"linear-gradient(90deg,#667eea,#764ba2,#4fc3f7)", marginBottom:28 }} />

        {error && (
          <div style={{ background:"rgba(244,114,182,0.1)", border:"1px solid rgba(244,114,182,0.3)",
            borderRadius:12, padding:"12px 16px", marginBottom:20, color:"#f472b6", fontSize:13 }}>
            ⚠️ {error}
          </div>
        )}
        {success && (
          <div style={{ background:"rgba(16,185,129,0.12)", border:"1px solid rgba(16,185,129,0.35)",
            borderRadius:12, padding:"12px 16px", marginBottom:20, color:"#86efac", fontSize:13 }}>
            ✅ {success}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ ...gridStyle, gridTemplateColumns:"1fr auto" }}>
            ...
            <div style={{ display:"flex", alignItems:"flex-end" }}>
              <button type="button" onClick={handleUseEquipmentData} disabled={!selectedAsset}
                style={{ padding:"11px 18px", borderRadius:8, border:"none",
                  background: selectedAsset ? "linear-gradient(135deg,#667eea,#764ba2)" : "rgba(255,255,255,0.08)",
                  color:"#fff", fontWeight:700,
                  cursor: selectedAsset ? "pointer" : "not-allowed", whiteSpace:"nowrap" }}>
          ...
          <div style={gridStyle}>
            ...
            <SelectField label="Equipment Status" name="equipment_status" value={form.equipment_status}
              onChange={handleChange}
              options={["PASS","FAIL","CONDITIONAL"].map(v=>({value:v,label:v}))} />
            <SelectField label="Record Status" name="status" value={form.status}
              onChange={handleChange}
          ...
          <div style={gridStyle}>
            ...
            {/* Lanyard Serial No — shown when equipment is harness/fall-arrest type */}
            {showLanyard && (
              <div>
                ...
                <input name="lanyard_serial_no" value={form.lanyard_serial_no} onChange={handleChange}
            ...
            <Field label="Inspector ID No." name="inspector_id"   value={form.inspector_id}   onChange={handleChange} />
          ...
          <div style={gridStyle}>
            <div>
              ...
              <p style={{ color:"rgba(255,255,255,0.4)", fontSize:12, marginTop:6 }}>
                {uploadingSignature ? "⏳ Uploading…" : "PNG / JPG / SVG — clear background recommended"}
              </p>
            </div>
            <div>
              <label style={labelStyle}>Signature URL</label>
              <input name="signature_url" value={form.signature_url} onChange={handleChange}
                placeholder="Auto-filled after upload" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Signature Preview</label>
              <div style={{ background:"#fff", borderRadius:8, padding:12, minHeight:90,
                display:"flex", alignItems:"center", justifyContent:"center",
                border:"1px solid #e2e8f0" }}>
                {form.signature_url ? (
                  <>
                    <img key={form.signature_url} src={form.signature_url} alt="Signature"
          ...
          {/* ── Actions ── */}
          <div style={{ display:"flex", gap:12, marginTop:32, marginBottom:40, flexWrap:"wrap" }}>
