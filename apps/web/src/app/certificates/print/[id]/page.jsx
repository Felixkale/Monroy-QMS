...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box}
  ...
  .pt-toolbar{position:sticky;top:0;z-index:100;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 20px;background:rgba(7,14,24,0.96);border-bottom:1px solid rgba(148,163,184,0.12);backdrop-filter:blur(16px);flex-wrap:wrap;-webkit-tap-highlight-color:transparent}
  .pt-btn-row{display:flex;gap:8px;flex-wrap:wrap;align-items:center}
  .pt-btn{display:inline-flex;align-items:center;justify-content:center;gap:7px;padding:10px 18px;border-radius:11px;font-family:'IBM Plex Sans',sans-serif;font-size:13px;font-weight:800;cursor:pointer;border:none;min-height:44px;min-width:44px;-webkit-tap-highlight-color:transparent;transition:filter .15s,transform .15s}
  ...
  .pt-progress{position:fixed;top:0;left:0;right:0;z-index:200;background:rgba(7,14,24,0.97);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:40px;min-height:100vh;font-family:'IBM Plex Sans',sans-serif;color:#f0f6ff}
  .pt-spinner{width:48px;height:48px;border-radius:50%;border:3px solid rgba(34,211,238,0.15);border-top-color:#22d3ee;animation:spin 0.8s linear infinite}
  @keyframes spin{to{transform:rotate(360deg)}}
  .pt-content{min-height:100vh;background:#d8dde3;padding:24px;display:flex;flex-direction:column;align-items:center}
  .pt-cert-wrap{background:#ffffff;padding:0;margin:0;display:block;width:794px;max-width:100%}
  .pt-cert-page{display:block;background:#ffffff;width:794px;max-width:100%}
  @media print{
    .pt-toolbar{display:none!important}
    .pt-content{background:white!important;padding:0!important;display:block!important}
    .pt-cert-wrap,.pt-cert-page{width:100%!important}
    body{background:white!important}
    @page{size:A4;margin:0}
  }
  @media(max-width:840px){.pt-cert-wrap,.pt-cert-page{width:100%}.pt-content{padding:12px}}
...
export default function CertificatePrintPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data.folder_id) {
        const { data: folder } = await supabase.from("certificates").select("*")
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const prevBg = el.style.background; el.style.background = "#ffffff"; await sleep(80);
        const blob = await window.html2pdf().set(pdfOpt("cert.pdf", el)).from(el).outputPdf("blob");
        ...
        await Promise.all(certs.map(async cert => {
          ...
          if (se) { console.warn("Upload failed:", se.message); return; }
      ...
      } catch (err) { console.warn("Auto-upload failed:", err.message); setUploadStatus(""); }
    }, 3000);
  ...
  // ── open cover-print — single URL, cover-print page handles pass/fail split ──
  function openCover() {
    ...
    <><style>{CSS}</style>
      <div style={{ minHeight:"100vh", background:T.bg, display:"flex", alignItems:"center", justifyContent:"center", padding:24, fontFamily:"'IBM Plex Sans',sans-serif" }}>
        <div style={{ background:T.redDim, border:`1px solid ${T.redBrd}`, borderRadius:14, padding:"20px 24px", color:T.red, fontSize:14, fontWeight:700, maxWidth:400, textAlign:"center" }}>
          <div style={{ fontSize:24, marginBottom:10 }}>⚠</div>{error}<br/>
          <button onClick={() => router.push(`/certificates/${id}`)} style={{ marginTop:14, padding:"9px 18px", borderRadius:10, border:"none", background:T.redDim, color:T.red, cursor:"pointer", fontWeight:700, fontFamily:"'IBM Plex Sans',sans-serif" }}>← Back to Certificates</button>
        </div>
      </div>
    </>
  );
    ...
    <><style>{CSS}</style>
      ...
      <div className="pt-toolbar">
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div className="pt-title">{isFolder ? `${clientName} — ${certs.length} Certificates` : certNumber}</div>
          {uploadStatus==="uploading" && (
            <span style={{ fontSize:10, color:T.textDim, display:"flex", alignItems:"center", gap:5 }}>
              <span style={{ width:8,height:8,borderRadius:"50%",border:"1.5px solid rgba(34,211,238,0.4)",borderTopColor:T.accent,animation:"spin 0.7s linear infinite",display:"inline-block" }}/>Storing…
            </span>
          )}
          {uploadStatus==="done" && <span style={{ fontSize:10, color:T.green }}>✓ Stored</span>}
        </div>
        <div className="pt-btn-row">
          <button type="button" className="pt-btn pt-btn-back" onClick={() => router.push(`/certificates/${id}`)} style={{ border:"1px solid rgba(148,163,184,0.18)" }}>← Back</button>
          <button type="button" className="pt-btn pt-btn-print" onClick={() => window.print()}>🖨 Print</button>
          {isFolder && (
          ...
          <button type="button" className="pt-btn pt-btn-save" onClick={handleSavePDF} disabled={saving||!html2pdfReady} title={!html2pdfReady?"Loading PDF engine…":"Save as PDF"}>
            {saving ? <><span style={{ width:14,height:14,borderRadius:"50%",border:"2px solid rgba(0,16,24,0.3)",borderTopColor:"#001018",animation:"spin 0.8s linear infinite",display:"inline-block" }}/> Saving…</> : saveMsg||"⬇ Save as PDF"}
          </button>
        </div>
      </div>
