...
const PRINT_CSS = `
  ...
  .cert-sheet {
    ...
    border-radius: 4px;
    overflow: hidden;
    /* Fit to one A4 page — use min-height only in screen mode */
    min-height: 1080px;
    display: flex;
    flex-direction: column;
  }

  /* When inside print mode, remove screen shadows */
  ...
  @media print {
    ...
    .cert-field-value { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .cert-header      { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .cert-accent-bar  { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .result-pill      { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  }

  /* ── SCREEN — dark wrapper for web view ── */
  .cert-sheet-wrap {
...
function Field({ label, value, mono = false, large = false, full = false }) {
  ...
  return (
    <div className={`cert-field${full ? " cert-field-full" : ""}`}>
...
function Section({ title, children, cols = 4 }) {
  return (
    <div className="cert-section">
      ...
      <div className={`cert-fields cert-fields-${cols === 3 ? "3" : cols === 2 ? "2" : cols === 1 ? "full" : ""}`}>
        {children}
      </div>
    </div>
  );
...
export default function CertificateSheet({ certificate: c, index = 0, total = 1, printMode = false }) {
  ...
  /* ✅ FIX: expiry_date = next_inspection_date — use whichever is set */
  ...
  /* Decide which technical section to show based on cert type */
        ...
        <div className={`cert-sheet${printMode ? " print-mode" : ""}`}>
          ...
          <div className="cert-body">
            ...
            <Section title="Certificate Details" cols={4}>
              ...
              {/* ✅ Label clarifies it is also the next inspection date */}
            ...
            {/* Technical — show relevant fields only */}
            {isPressure ? (
          ...
          <div className="cert-footer">
            <div className="cert-footer-grid">
              ...
              {/* Stamp / Company seal placeholder */}
