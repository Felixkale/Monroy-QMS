...
export default function RegisterClientPage() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    const { data, error: saveError } = await registerClient(form);
    setSaving(false);
    if (saveError) {
      setError(typeof saveError === "string" ? saveError : saveError.message || "Failed to register client.");
      return;
    }
    setSuccess(true);
  ...
  return (
    <AppLayout title="Register Client">
        ...
        <div style={{ background:"linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))", border:"1px solid rgba(124,92,252,0.2)", borderRadius:16, padding:"24px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div>
              ...
              <input style={inputStyle} value={form.company_name} onChange={e => set("company_name", e.target.value)} placeholder="e.g. Kgalagadi Mining (Pty) Ltd"
                onFocus={e => e.target.style.borderColor=C.purple} onBlur={e => e.target.style.borderColor="rgba(124,92,252,0.25)"} />
            </div>
            <div>
              <label style={labelStyle}>Company Code</label>
              <input style={inputStyle} value={form.company_code} onChange={e => set("company_code", e.target.value)} placeholder="e.g. KGM-001"
                onFocus={e => e.target.style.borderColor=C.purple} onBlur={e => e.target.style.borderColor="rgba(124,92,252,0.25)"} />
            </div>
            <div>
            ...
            <div>
              ...
              <input style={inputStyle} value={form.address} onChange={e => set("address", e.target.value)} placeholder="e.g. Plot 1234, Tlokweng Road"
                onFocus={e => e.target.style.borderColor=C.purple} onBlur={e => e.target.style.borderColor="rgba(124,92,252,0.25)"} />
            </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div>
              ...
              <input style={inputStyle} value={form.contact_person} onChange={e => set("contact_person", e.target.value)} placeholder="Full name"
                onFocus={e => e.target.style.borderColor=C.blue} onBlur={e => e.target.style.borderColor="rgba(124,92,252,0.25)"} />
            </div>
            <div>
              <label style={labelStyle}>Contact Email *</label>
              <input style={inputStyle} type="email" value={form.contact_email} onChange={e => set("contact_email", e.target.value)} placeholder="email@company.co.bw"
                onFocus={e => e.target.style.borderColor=C.blue} onBlur={e => e.target.style.borderColor="rgba(124,92,252,0.25)"} />
            </div>
            <div>
              <label style={labelStyle}>Contact Phone</label>
              <input style={inputStyle} value={form.contact_phone} onChange={e => set("contact_phone", e.target.value)} placeholder="+267 3900 000"
                onFocus={e => e.target.style.borderColor=C.blue} onBlur={e => e.target.style.borderColor="rgba(124,92,252,0.25)"} />
            </div>
          </div>
      ...
      <div style={{ display:"flex", gap:12, marginTop:24, justifyContent:"flex-end" }}>
        <a href="/clients" style={{ padding:"12px 24px", borderRadius:12, textDecoration:"none", background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)", color:"#94a3b8", fontSize:14, fontWeight:600 }}>
          Cancel
        </a>
        <button onClick={handleSubmit} disabled={saving || success} style={{
          ...
          opacity: saving ? 0.7 : 1, boxShadow: saving ? "none" : `0 0 20px rgba(124,92,252,0.4)`, transition:"all 0.2s",
        }}>
