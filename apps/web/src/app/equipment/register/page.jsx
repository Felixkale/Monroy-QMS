...
// ── Sidebar (defined OUTSIDE main component to prevent remount on re-render) ──
...
// ── Form (defined OUTSIDE main component to prevent remount on re-render) ─────
function RegistrationForm({ formData, onChange, onSubmit, onClear, loading, submitError }) {
    ...
    <form onSubmit={onSubmit} style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <input style={inputStyle} type="text" name="tag" placeholder="e.g. PV-0042" value={formData.tag} onChange={onChange} required onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Serial Number *</label>
          <input style={inputStyle} type="text" name="serial" placeholder="e.g. S-10042" value={formData.serial} onChange={onChange} required onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Equipment Type *</label>
          <select style={{ ...inputStyle, cursor: "pointer" }} name="equipment_type" value={formData.equipment_type} onChange={onChange}>
            {EQUIPMENT_TYPES.map(t => <option key={t}>{t}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Manufacturer *</label>
          <input style={inputStyle} type="text" name="manufacturer" placeholder="e.g. ASME Corp" value={formData.manufacturer} onChange={onChange} required onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Model / Drawing No.</label>
          <input style={inputStyle} type="text" name="model" placeholder="Model or drawing ref" value={formData.model} onChange={onChange} onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Year Built</label>
          <input style={inputStyle} type="number" name="year_built" placeholder="2024" value={formData.year_built} onChange={onChange} onFocus={focus} onBlur={blur} />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <input style={inputStyle} type="text" name="client" placeholder="Client / company name" value={formData.client} onChange={onChange} required onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Location / Town *</label>
          <BotswanaLocationPicker name="location" value={formData.location} onChange={onChange} required />
        </div>
        <div>
          <label style={labelStyle}>Department / Plant</label>
          <input style={inputStyle} type="text" name="department" placeholder="e.g. Plant A – Bay 3" value={formData.department} onChange={onChange} onFocus={focus} onBlur={blur} />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <select style={{ ...inputStyle, cursor: "pointer" }} name="cert_type" value={formData.cert_type} onChange={onChange}>
            {CERT_TYPES.map(c => <option key={c}>{c}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Design Standard *</label>
          <select style={{ ...inputStyle, cursor: "pointer" }} name="design_standard" value={formData.design_standard} onChange={onChange}>
            {STANDARDS.map(s => <option key={s}>{s}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Inspection Frequency</label>
          <select style={{ ...inputStyle, cursor: "pointer" }} name="inspection_freq" value={formData.inspection_freq} onChange={onChange}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <select style={{ ...inputStyle, cursor: "pointer" }} name="shell_material" value={formData.shell_material} onChange={onChange}>
            {MATERIALS.map(m => <option key={m}>{m}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Fluid / Contents Type</label>
          <select style={{ ...inputStyle, cursor: "pointer" }} name="fluid_type" value={formData.fluid_type} onChange={onChange}>
        ...
        {!isLifting && <>
          <div>
            ...
            <input style={inputStyle} type="number" step="0.01" name="design_pressure" placeholder="e.g. 15.0" value={formData.design_pressure} onChange={onChange} onFocus={focus} onBlur={blur} />
          </div>
          <div>
            <label style={labelStyle}>Working Pressure (kPa)</label>
            <input style={inputStyle} type="number" step="0.01" name="working_pressure" placeholder="e.g. 1000" value={formData.working_pressure} onChange={onChange} onFocus={focus} onBlur={blur} />
          </div>
          <div>
            <label style={labelStyle}>Test Pressure (kPa)</label>
            <input style={inputStyle} type="number" step="0.01" name="test_pressure" placeholder="e.g. 1500" value={formData.test_pressure} onChange={onChange} onFocus={focus} onBlur={blur} />
          </div>
          <div>
            <label style={labelStyle}>Design Temperature (°C)</label>
            <input style={inputStyle} type="number" step="0.1" name="design_temperature" placeholder="e.g. 250" value={formData.design_temperature} onChange={onChange} onFocus={focus} onBlur={blur} />
          </div>
          <div>
            <label style={labelStyle}>Volume / Capacity (L)</label>
            <input style={inputStyle} type="number" step="0.1" name="capacity_volume" placeholder="e.g. 5000" value={formData.capacity_volume} onChange={onChange} onFocus={focus} onBlur={blur} />
          </div>
        </>}
        {isLifting && (
          <div>
            <label style={labelStyle}>Safe Working Load (kg)</label>
            <input style={inputStyle} type="number" step="1" name="safe_working_load" placeholder="e.g. 5000" value={formData.safe_working_load} onChange={onChange} onFocus={focus} onBlur={blur} />
          </div>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <input style={inputStyle} type="text" name="national_reg_no" placeholder="e.g. BW-PV-2024-00123" value={formData.national_reg_no} onChange={onChange} onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Notified Body / Inspector</label>
          <input style={inputStyle} type="text" name="notified_body" placeholder="Inspecting authority" value={formData.notified_body} onChange={onChange} onFocus={focus} onBlur={blur} />
        </div>
        <div>
...
// ── Print certificate in new window ──────────────────────────────────────────
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .header-left .authority { font-size:9px; font-weight:700; color:#667eea; letter-spacing:0.15em; text-transform:uppercase; margin-bottom:6px; }
    .header-left h1 { font-size:24px; font-weight:900; color:#0f172a; }
    .header-left .standard { font-size:11px; color:#64748b; }
    .header-left .standard span { color:#667eea; font-weight:600; }
    .header-right { text-align:right; }
    .header-right .cert-no { font-size:16px; font-weight:900; color:#059669; font-family:monospace; }
    .header-right .cert-date { font-size:10px; color:#94a3b8; margin-top:4px; }
    .status { background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; padding:12px 16px; margin-bottom:24px; display:flex; align-items:center; gap:10px; }
    .status-title { font-size:13px; font-weight:700; color:#16a34a; }
    .status-sub { font-size:11px; color:#64748b; margin-top:2px; }
    .section-title { font-size:10px; font-weight:800; color:#667eea; text-transform:uppercase; letter-spacing:0.12em; border-bottom:1px solid #e2e8f0; padding-bottom:6px; margin:20px 0 10px; }
    ...
    .signatures { display:grid; grid-template-columns:1fr 1fr 1fr; gap:20px; margin-top:40px; padding-top:16px; border-top:1px solid #e2e8f0; }
    .sig-line { border-top:1px solid #cbd5e1; padding-top:8px; margin-top:40px; font-size:9px; color:#94a3b8; text-transform:uppercase; letter-spacing:0.1em; text-align:center; }
    .footer { margin-top:24px; padding-top:12px; border-top:1px dashed #e2e8f0; text-align:center; font-size:9px; color:#cbd5e1; line-height:1.6; }
    @media print { .no-print { display:none !important; } }
    .print-bar { position:fixed; bottom:0; left:0; right:0; background:#fff; border-top:1px solid #e2e8f0; padding:14px 24px; display:flex; gap:10px; justify-content:flex-end; }
...
<body>
  ...
  <div class="header">
    ...
    <div class="header-right">
      <div style="font-size:9px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:4px;">Certificate No.</div>
  ...
  ${data.notes ? `<div class="section-title">📝 Remarks</div><p style="font-size:12px;color:#334155;line-height:1.7;padding:10px;background:#f8fafc;border-radius:6px;">${data.notes}</p>` : ""}
  <div class="signatures">
    ${["Registered By","Approved By","Inspecting Authority"].map(r => `<div class="sig-line">${r}</div>`).join("")}
  </div>
  <div class="footer">
...
function CertificateModal({ data, certNo, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
      <div style={{ background: "linear-gradient(160deg,#1a1f2e,#0f1419)", border: "1px solid rgba(102,126,234,0.35)", borderRadius: 20, width: "100%", maxWidth: 680, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 0 60px rgba(102,126,234,0.2)" }}>
        <div style={{ background: "linear-gradient(135deg,rgba(102,126,234,0.15),rgba(118,75,162,0.1))", borderBottom: "1px solid rgba(102,126,234,0.2)", padding: "26px 30px 20px", borderRadius: "20px 20px 0 0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: 9, fontWeight: 800, color: "#667eea", letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: 6 }}>Republic of Botswana · Pressure Equipment Directorate</div>
              <h2 style={{ margin: 0, fontSize: 22, fontWeight: 900, color: "#fff" }}>{data.cert_type || "Inspection Certificate"}</h2>
              <div style={{ marginTop: 5, fontSize: 11, color: "rgba(255,255,255,0.5)" }}>Issued under <span style={{ color: C.blue }}>{data.design_standard}</span></div>
            </div>
        ...
        <div style={{ padding: "24px 30px" }}>
          <div style={{ background: "rgba(0,245,196,0.07)", border: "1px solid rgba(0,245,196,0.2)", borderRadius: 10, padding: "11px 16px", marginBottom: 22, display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 20 }}>✅</span>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: C.green }}>Certificate Valid — Equipment Successfully Registered</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>{data.tag} · {data.equipment_type} · {data.location}</div>
            </div>
          </div>
          ...
          <Row k="Owner / Client" v={data.client} /><Row k="Installation Location" v={data.location} />
          ...
          {data.notes && <>
            ...
            <div style={{ fontSize: 12, color: "#e2e8f0", lineHeight: 1.7, background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: "10px 14px" }}>{data.notes}</div>
          </>}

          <div style={{ marginTop: 28, borderTop: "1px solid rgba(102,126,234,0.15)", paddingTop: 20, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            {["Registered By", "Approved By", "Inspecting Authority"].map(role => (
              <div key={role} style={{ textAlign: "center" }}>
                <div style={{ borderTop: "1px solid rgba(255,255,255,0.15)", paddingTop: 8, marginTop: 32, fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.1em" }}>{role}</div>
              </div>
            ))}
          </div>

          <div style={{ marginTop: 20, textAlign: "center", fontSize: 10, color: "rgba(100,116,139,0.45)", lineHeight: 1.6, borderTop: "1px dashed rgba(102,126,234,0.1)", paddingTop: 14 }}>
            Computer-generated certificate · Cert No. {certNo} · {today}<br />
            Verify at the Botswana Pressure Equipment Directorate portal.
          </div>
        </div>

        <div style={{ position: "sticky", bottom: 0, background: "#1a1f2e", borderTop: "1px solid rgba(102,126,234,0.15)", padding: "14px 24px", display: "flex", gap: 10, justifyContent: "flex-end", borderRadius: "0 0 20px 20px" }}>
          ...
          <button onClick={() => printCertificate(data, certNo, today)} style={{ padding: "9px 22px", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: 13, background: "linear-gradient(135deg,#667eea,#764ba2)", border: "none", color: "#fff", boxShadow: "0 0 18px rgba(102,126,234,0.35)" }}>🖨 Preview & Save PDF</button>
        </div>
      </div>
    </div>
  );
...
export default function RegisterEquipmentPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error: equipError } = await supabase.from("equipment").insert([formData]);
      if (equipError) throw equipError;
      const { error: certError } = await supabase.from("certificates").insert([{ ...formData, cert_no: certNo }]);
