...
export default function RegisterEquipmentPage() {
  ...
  // ── Form state — field names match Supabase columns exactly ──────────────
  const emptyForm = {
    // Identity
    tag:                "",
    serial:             "",
    ...
    department:         "",
    // Certificate info
    cert_type:          "Inspection Certificate",
    ...
    test_pressure:      "",
    design_temperature: "",
    capacity_volume:    "",          // ← matches Supabase column name
    safe_working_load:  "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Insert into certificates table with cert_no + all form fields
      const { error: certError } = await supabase
        .from("certificates")
        .insert([{ ...formData, cert_no: certNo }]);
      if (certError) throw certError;

      // 3. Show the generated certificate modal
      ...
      <div style={{ padding: "16px 12px", borderTop: "1px solid rgba(102,126,234,0.1)" }}>
        ...
        <button
          style={{
            ...
            fontWeight: 600, cursor: "pointer", transition: "all .25s",
            fontFamily: "inherit",
          }}
    ...
    <form onSubmit={handleSubmit} style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <input style={inputStyle} type="text" name="tag"
            placeholder="e.g. PV-0042" value={formData.tag}
            onChange={handleChange} required onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Serial Number *</label>
          <input style={inputStyle} type="text" name="serial"
            placeholder="e.g. S-10042" value={formData.serial}
            onChange={handleChange} required onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Equipment Type *</label>
          <select style={{ ...inputStyle, cursor: "pointer" }} name="equipment_type"
            value={formData.equipment_type} onChange={handleChange}>
        ...
        <div>
          ...
          <input style={inputStyle} type="text" name="manufacturer"
            placeholder="e.g. ASME Corp" value={formData.manufacturer}
            onChange={handleChange} required onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Model / Drawing No.</label>
          <input style={inputStyle} type="text" name="model"
            placeholder="Model or drawing ref" value={formData.model}
            onChange={handleChange} onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Year Built</label>
          <input style={inputStyle} type="number" name="year_built"
            placeholder="2024" value={formData.year_built}
            onChange={handleChange} onFocus={focus} onBlur={blur} />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <input style={inputStyle} type="text" name="client"
            placeholder="Client / company name" value={formData.client}
            onChange={handleChange} required onFocus={focus} onBlur={blur} />
        </div>
        <div>
        ...
        <div>
          ...
          <input style={inputStyle} type="text" name="department"
            placeholder="e.g. Plant A – Bay 3" value={formData.department}
            onChange={handleChange} onFocus={focus} onBlur={blur} />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <select style={{ ...inputStyle, cursor: "pointer" }} name="cert_type"
            value={formData.cert_type} onChange={handleChange}>
            {CERT_TYPES.map(c => <option key={c}>{c}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Design Standard *</label>
          <select style={{ ...inputStyle, cursor: "pointer" }} name="design_standard"
            value={formData.design_standard} onChange={handleChange}>
            {STANDARDS.map(s => <option key={s}>{s}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Inspection Frequency</label>
          <select style={{ ...inputStyle, cursor: "pointer" }} name="inspection_freq"
            value={formData.inspection_freq} onChange={handleChange}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <select style={{ ...inputStyle, cursor: "pointer" }} name="shell_material"
            value={formData.shell_material} onChange={handleChange}>
            {MATERIALS.map(m => <option key={m}>{m}</option>)}
          </select>
        </div>
        <div>
          <label style={labelStyle}>Fluid / Contents Type</label>
          <select style={{ ...inputStyle, cursor: "pointer" }} name="fluid_type"
            value={formData.fluid_type} onChange={handleChange}>
        ...
        {!isLifting && <>
          <div>
            ...
            <input style={inputStyle} type="number" step="0.01" name="design_pressure"
              placeholder="e.g. 15.0" value={formData.design_pressure}
              onChange={handleChange} onFocus={focus} onBlur={blur} />
          </div>
          <div>
            <label style={labelStyle}>Working Pressure (kPa)</label>
            <input style={inputStyle} type="number" step="0.01" name="working_pressure"
              placeholder="e.g. 1000" value={formData.working_pressure}
              onChange={handleChange} onFocus={focus} onBlur={blur} />
          </div>
          <div>
            <label style={labelStyle}>Test Pressure (kPa)</label>
            <input style={inputStyle} type="number" step="0.01" name="test_pressure"
              placeholder="e.g. 1500" value={formData.test_pressure}
              onChange={handleChange} onFocus={focus} onBlur={blur} />
          </div>
          <div>
            <label style={labelStyle}>Design Temperature (°C)</label>
            <input style={inputStyle} type="number" step="0.1" name="design_temperature"
              placeholder="e.g. 250" value={formData.design_temperature}
              onChange={handleChange} onFocus={focus} onBlur={blur} />
          </div>
          <div>
            <label style={labelStyle}>Volume / Capacity (L)</label>
            {/* name="capacity_volume" matches the Supabase column exactly */}
            <input style={inputStyle} type="number" step="0.1" name="capacity_volume"
              placeholder="e.g. 5000" value={formData.capacity_volume}
              onChange={handleChange} onFocus={focus} onBlur={blur} />
          </div>
        </>}
        ...
        {isLifting && (
          <div>
            ...
            <input style={inputStyle} type="number" step="1" name="safe_working_load"
              placeholder="e.g. 5000" value={formData.safe_working_load}
              onChange={handleChange} onFocus={focus} onBlur={blur} />
          </div>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 24 }}>
        <div>
          ...
          <input style={inputStyle} type="text" name="national_reg_no"
            placeholder="e.g. BW-PV-2024-00123" value={formData.national_reg_no}
            onChange={handleChange} onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Notified Body / Inspector</label>
          <input style={inputStyle} type="text" name="notified_body"
            placeholder="Inspecting authority" value={formData.notified_body}
            onChange={handleChange} onFocus={focus} onBlur={blur} />
        </div>
        <div>
          <label style={labelStyle}>Installation Date</label>
          <input style={inputStyle} type="date" name="installation_date"
            value={formData.installation_date} onChange={handleChange} />
        </div>
        <div>
          <label style={labelStyle}>Last Inspection Date</label>
          <input style={inputStyle} type="date" name="last_inspection_date"
            value={formData.last_inspection_date} onChange={handleChange} />
        </div>
        <div>
          <label style={labelStyle}>Next Inspection Due</label>
          <input style={inputStyle} type="date" name="next_inspection_date"
            value={formData.next_inspection_date} onChange={handleChange} />
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#0f1419", color: "#e2e8f0" }}>
      ...
      {/* Auto-generated certificate modal */}
      {certData && (
        <CertificateModal
          data={certData}
