  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <h2 style={{ fontSize: 14, fontWeight: 900, margin: 0, color: "#fff" }}>
            Monroy
          </h2>
      ...
      >
        <div>
          ...
          <input
            style={inputStyle}
            type="text"
            name="national_reg_no"
            value={formData.national_reg_no}
            onChange={onChange}
            onFocus={focus}
            onBlur={blur}
          />
...
function printCertificate(data, certNo, today) {
  const isPressureEquipment = PRESSURE_EQUIPMENT_TYPES.includes(data.equipment_type);
...
<body>
  ...
  <div class="header">
    <div>
      <div style="font-size:10px;font-weight:700;color:#667eea;text-transform:uppercase;">
        Republic of Botswana · Pressure Equipment Directorate
      </div>
      <h1 style="margin:6px 0 0;font-size:24px;">
        ${data.cert_type || "Inspection Certificate"}
      </h1>
      <div style="font-size:12px;color:#6b7280;">
        Issued under ${data.design_standard || ""}
      </div>
    </div>
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 900, color: C.green, fontFamily: "monospace" }}>
                {certNo}
              </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const assetPayload = {
        ...
        asset_type: normalizeText(formData.equipment_type),
        serial_number: normalizeText(formData.serial_number),
        manufacturer: normalizeText(formData.manufacturer),
        model: normalizeText(formData.model),
        description: normalizeText(formData.notes)
          ? `${formData.equipment_type} | ${formData.notes.trim()}`
          : normalizeText(formData.equipment_type),
        location: normalizeText(formData.location),
        condition: "Good",
        status: "active",
        year_built: normalizeNumber(formData.year_built),
        department: normalizeText(formData.department),
        cert_type: normalizeText(formData.cert_type),
        design_standard: normalizeText(formData.design_standard),
        inspection_freq: normalizeText(formData.inspection_freq),
        shell_material: normalizeText(formData.shell_material),
        fluid_type: normalizeText(formData.fluid_type),
        design_pressure: normalizeNumber(formData.design_pressure),
        working_pressure: normalizeNumber(formData.working_pressure),
        test_pressure: normalizeNumber(formData.test_pressure),
        design_temperature: normalizeNumber(formData.design_temperature),
        capacity_volume: normalizeNumber(formData.capacity_volume),
        safe_working_load: normalizeNumber(formData.safe_working_load),
        national_reg_no: normalizeText(formData.national_reg_no),
        notified_body: normalizeText(formData.notified_body),
        installation_date: normalizeText(formData.installation_date),
        last_inspection_date: normalizeText(formData.last_inspection_date),
        next_inspection_date: normalizeText(formData.next_inspection_date),
      ...
      const certificatePayload = {
        ...
        equipment_location: normalizeText(formData.location),
        ...
        valid_to: normalizeText(formData.next_inspection_date),
        status: "issued",
        legal_framework: normalizeText(formData.design_standard),
      };
