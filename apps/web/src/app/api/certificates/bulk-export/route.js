...
const CSS = `
  *,*::before,*::after{box-sizing:border-box}
  ...
  /* Off-screen render container — visible to html2pdf but not user */
  #cert-offscreen{
    position:fixed;
    left:-9999px;
    top:0;
...
export default function BulkExportClient() {
  ...
  // ── Render one cert to ArrayBuffer via html2pdf ──────────────────────────
  function captureOneCert(cert) {
    return new Promise((resolve, reject) => {
      // Put cert into the off-screen div
      setRenderCert(cert);
      ...
      setTimeout(async () => {
        try {
          ...
          // html2pdf needs the actual DOM element
          const opt = {
            margin:      0,
          ...
          // Use .output("arraybuffer") — most reliable for binary data
      ...
      }, 1000); // 1s for React + fonts to render
    });
  }
  ...
  async function handleExport() {
    ...
    try {
      // Fetch full cert data in batches
      ...
      for (const cert of allCerts) {
        ...
        try {
          ...
          // Add ArrayBuffer directly to zip — no blob conversion needed
          zip.file(`${clientFolder}/${safeDate}_${safeCertNum}.pdf`, ab);
        } catch (e) {
          console.error(`Failed: ${cert.certificate_number}:`, e.message);
        }
      ...
      if (zipBlob.size < 100) throw new Error("ZIP file is empty — no PDFs were generated.");
  ...
  return (
    <AppLayout title="Bulk Export">
      ...
      {/* Off-screen render area for html2pdf */}
