...
export default function InspectionsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (inspRes.error) setError("Failed to load inspections.");
  ...
  const filtered = inspections.filter(i => {
    const matchFilter = filter === "All" || i.result === filter.toLowerCase();
    ...
    <AppLayout title={title}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12, marginBottom:24 }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <a href="/inspections/new" style={{ padding:"10px 18px", borderRadius:12, textDecoration:"none", background:`linear-gradient(135deg,${C.purple},${C.blue})`, color:"#fff", fontWeight:700, fontSize:13, boxShadow:`0 0 20px rgba(124,92,252,0.4)` }}>
            + New Inspection
          </a>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:16, alignItems:"center" }}>
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search by ID, equipment or client…"
      ...
      {loading && (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:14 }}>
          {Array.from({length:4}).map((_,i) => (
            <div key={i} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:14, padding:"18px 20px", height:130 }} />
          ))}
        </div>
      )}
          ...
          {filtered.map(insp => {
            ...
            return (
              <a key={insp.id} href={`/inspections/${insp.id}`} style={{ textDecoration:"none" }}>
                <div
                  style={{ background:"linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))", border:`1px solid rgba(${rrgb},0.2)`, borderRadius:14, padding:"18px 20px", cursor:"pointer", transition:"all 0.2s", position:"relative", overflow:"hidden" }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor=`rgba(${rrgb},0.5)`; e.currentTarget.style.transform="translateY(-2px)"; e.currentTarget.style.background=`rgba(${rrgb},0.06)`; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor=`rgba(${rrgb},0.2)`; e.currentTarget.style.transform="translateY(0)"; e.currentTarget.style.background="linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))"; }}
                >
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                    <div>
                      <div style={{ fontSize:15, fontWeight:800, color:"#fff" }}>{insp.inspection_number || insp.id?.slice(0,8)}</div>
                      <div style={{ fontSize:11, color:"#64748b" }}>{insp.inspection_date ? new Date(insp.inspection_date).toLocaleDateString() : "—"}</div>
                    </div>
                    {insp.result && (
                      <span style={{ padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:700, background:`rgba(${rrgb},0.15)`, color:rc, border:`1px solid rgba(${rrgb},0.3)`, textTransform:"capitalize" }}>
                  ...
                  {insp.notes && (
                    <div style={{ fontSize:11, color:"#475569", marginTop:8, fontStyle:"italic" }}>{insp.notes.slice(0,80)}{insp.notes.length > 80 ? "…" : ""}</div>
                  )}
                </div>
              </a>
            );
