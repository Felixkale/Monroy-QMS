        ...
        {filtered.map(insp=>(
          ...
          onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(124,92,252,0.15)"}>
            ...
            <div style={{ minWidth:120 }}>
              ...
              <span style={{
                display:"inline-block", marginTop:4, padding:"2px 8px", borderRadius:20, fontSize:10, fontWeight:700,
                background:`rgba(${rgbaMap[typeColor[insp.type]]||"124,92,252"},0.15)`,
                color: typeColor[insp.type] || C.purple,
                border:`1px solid rgba(${rgbaMap[typeColor[insp.type]]||"124,92,252"},0.3)`,
              }}>{insp.type}</span>
            </div>
            {/* Middle */}
            <div>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", justifyContent:"flex-end" }}>
              <span style={{
                ...
                background:`rgba(${resultColor[insp.result]===C.green?"0,245,196":resultColor[insp.result]===C.pink?"244,114,182":"251,191,36"},0.12)`,
                color: resultColor[insp.result],
                border:`1px solid rgba(${resultColor[insp.result]===C.green?"0,245,196":resultColor[insp.result]===C.pink?"244,114,182":"251,191,36"},0.3)`,
              }}>{insp.result}</span>
              <span style={{
                padding:"4px 12px", borderRadius:20, fontSize:11, fontWeight:700,
                background:`rgba(${statusColor[insp.status]===C.green?"0,245,196":statusColor[insp.status]===C.blue?"79,195,247":"251,191,36"},0.12)`,
                color: statusColor[insp.status],
                border:`1px solid rgba(${statusColor[insp.status]===C.green?"0,245,196":statusColor[insp.status]===C.blue?"79,195,247":"251,191,36"},0.3)`,
              }}>{insp.status}</span>
            </div>
          </div>
        ))}
