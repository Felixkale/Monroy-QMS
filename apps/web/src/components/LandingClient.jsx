...
// ─────────────────────────────────────────────────────────────
//  Client component — receives pre-fetched stats as props
//  from the server component (page.jsx)
// ─────────────────────────────────────────────────────────────
"use client";
...
/* ── Stat card ── */
function StatCard({ label, value, sub, color, glow, dim, brd, icon }) {
  const displayed = CountUp({ target: value });
  return (
    <div style={{ background:dim, border:`1px solid ${brd}`, borderRadius:14, padding:"20px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ fontSize:44, fontWeight:900, color, lineHeight:1, marginBottom:6, fontFamily:"'IBM Plex Mono',monospace" }}>
        {displayed}
      </div>
...
export default function LandingClient({ stats }) {
  ...
  /* bar width helper — scales relative to largest value so bars are meaningful */
  const maxVal = Math.max(stats.clients, stats.equipment, stats.certificates, stats.openNcrs, stats.expiringSoon, 1);
  const barPct = v => Math.max(Math.round((v / maxVal) * 100), v > 0 ? 4 : 0);
    ...
    <>
      ...
      <section className="lp-hero">
        <div style={{ maxWidth:960 }}>
          <div className="lp-badge"><div className="lp-dot"/>Southern Africa's Industrial QMS Platform</div>
          ...
          {/* LIVE STAT CARDS — real numbers from DB */}
          ...
          <div className="lp-how-grid">
            <div>
              ...
              {STEPS.map(s => (
                <div key={s.n} className="lp-step lp-reveal">
                  ...
                  <div><div className="lp-step-t">{s.t}</div><p className="lp-step-d">{s.d}</p></div>
                </div>
              ))}
            </div>

            {/* Dashboard panel — real live data */}
            <div className="lp-dash lp-reveal">
              <div className="lp-dash-top">
                ...
                <span className="lp-dash-lbl">// Monroy QMS — Live</span>
                <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color:T.green }}>● live</span>
              </div>
              <div className="lp-dash-body">
                {[
                  { label:"Clients Registered",  val:stats.clients,      color:T.accent },
                  { label:"Equipment Tracked",    val:stats.equipment,    color:T.green  },
                  { label:"Certificates Issued",  val:stats.certificates, color:T.blue   },
                  { label:"Open NCRs",            val:stats.openNcrs,     color:T.red    },
                  { label:"Expiring ≤ 30 days",   val:stats.expiringSoon, color:T.amber  },
                ].map(b => (
                  <div key={b.label} className="lp-brow">
                    <div className="lp-blbl">
                      <span>{b.label}</span>
                      <b style={{ color:b.color }}>{b.val}</b>
                    </div>
                    <div className="lp-btrack">
                      <div className="lp-bfill" style={{ width:`${barPct(b.val)}%`, background:b.color }}/>
                    </div>
                  </div>
                ))}
              </div>
              <div className="lp-dash-mets">
                {[
                  { v:stats.certificates, l:"Certs",     c:T.green  },
