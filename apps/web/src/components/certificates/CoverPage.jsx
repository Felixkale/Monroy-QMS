...
const COVER_CSS = `
  ...
  .cv-page{background:#fff;width:210mm;height:297mm;display:flex;flex-direction:column;font-family:'IBM Plex Sans',sans-serif;color:#0f1923;box-shadow:0 8px 40px rgba(0,0,0,0.28);overflow:hidden;position:relative}
  .cv-page.pm{box-shadow:none;width:100%;height:297mm}
  .cv-hdr{background:#0b1d3a;position:relative;overflow:hidden;flex-shrink:0;min-height:130px}
  .cv-geo{position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none}
  .cv-hdr-inner{position:relative;z-index:2;display:flex;align-items:stretch;min-height:130px}
  .cv-logo-box{background:#fff;width:150px;flex-shrink:0;display:flex;align-items:center;justify-content:center;padding:16px;position:relative}
  .cv-logo-box::after{content:'';position:absolute;right:-28px;top:0;width:0;height:0;border-top:65px solid #fff;border-bottom:65px solid #fff;border-right:28px solid transparent}
  .cv-logo-box img{width:118px;height:118px;object-fit:contain}
  .cv-hdr-text{flex:1;padding:24px 24px 24px 52px;display:flex;flex-direction:column;justify-content:center}
  .cv-brand{font-size:8.5px;font-weight:800;letter-spacing:.22em;text-transform:uppercase;color:#4fc3f7;margin-bottom:6px}
  .cv-company{font-size:11px;font-weight:500;color:rgba(255,255,255,0.6);letter-spacing:.04em}
  .cv-hdr-contact{padding:20px 24px;display:flex;flex-direction:column;align-items:flex-end;justify-content:center;gap:5px;flex-shrink:0}
  .cv-cr{font-size:8px;color:rgba(255,255,255,0.65)}
  .cv-accent{height:5px;background:linear-gradient(90deg,#22d3ee 0%,#3b82f6 55%,#a78bfa 100%);flex-shrink:0}
  .cv-body{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 44px;position:relative;overflow:hidden}
  ...
  .cv-client-badge{background:#0b1d3a;border:1.5px solid #22d3ee;border-radius:6px;padding:10px 32px;display:inline-flex;flex-direction:column;align-items:center;margin-bottom:30px}
  .cv-client-label{font-size:7.5px;font-weight:800;letter-spacing:.22em;text-transform:uppercase;color:#4fc3f7;margin-bottom:4px}
  .cv-client-name{font-size:18px;font-weight:900;color:#fff;letter-spacing:.1em}
  .cv-year{width:116px;height:116px;border-radius:50%;background:#0b1d3a;display:flex;flex-direction:column;align-items:center;justify-content:center;margin:0 auto 28px;box-shadow:0 0 0 7px rgba(34,211,238,0.12),0 0 0 14px rgba(34,211,238,0.05)}
  .cv-year-label{font-size:7px;font-weight:800;letter-spacing:.18em;text-transform:uppercase;color:#4fc3f7;margin-bottom:2px}
  .cv-year-num{font-size:34px;font-weight:900;color:#fff;line-height:1;font-family:'IBM Plex Mono',monospace}
  .cv-title-sub{font-size:10px;font-weight:700;letter-spacing:.28em;text-transform:uppercase;color:#3b6ea5;margin-bottom:10px}
  .cv-title{font-size:40px;font-weight:900;letter-spacing:-0.03em;color:#0b1d3a;line-height:1.08;margin-bottom:14px}
  .cv-title-line{width:80px;height:4px;background:linear-gradient(90deg,#22d3ee,#3b82f6);border-radius:2px;margin:0 auto 28px}
  .cv-info-strip{display:flex;gap:0;border:1px solid #1e3a5f;border-radius:8px;overflow:hidden;width:100%;margin-bottom:16px}
  .cv-info-cell{flex:1;padding:12px 16px;border-right:1px solid #1e3a5f;background:#f4f8ff}
  .cv-info-cell:last-child{border-right:none}
  .cv-info-cell:nth-child(even){background:#eef4ff}
  .cv-info-label{font-size:7.5px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#3b6ea5;margin-bottom:4px}
  .cv-info-value{font-size:11.5px;font-weight:700;color:#0b1d3a}
  .cv-prepared{border:1px solid #1e3a5f;border-radius:8px;background:#0b1d3a;display:grid;grid-template-columns:1fr 1px 1fr;width:100%;overflow:hidden}
  .cv-prep-cell{padding:13px 20px;display:flex;align-items:center;justify-content:space-between;gap:12px}
  .cv-prep-divider{background:rgba(34,211,238,0.25)}
  .cv-prep-label{font-size:7.5px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:#4fc3f7;margin-bottom:3px}
  .cv-prep-name{font-size:13px;font-weight:900;color:#fff}
  .cv-prep-role{font-size:9px;color:rgba(255,255,255,0.5);margin-top:2px}
  .cv-prep-sig{height:38px;display:flex;align-items:center}
  .cv-prep-sig img{max-height:38px;max-width:96px;object-fit:contain;filter:brightness(10)}
